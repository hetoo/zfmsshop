...
$(function() {    
    ...
    $.ajax({
        type: 'post',
        url: ApiUrl + "/index.php?act=distribute&op=get_distri_info",
        data: {dis_id: dis_id},
        dataType: 'json',
        success: function(result) {
            var data = result.datas;
            //显示页面title
            var title = data.member_info.member_name + ' - 微分销';
            document.title = title;
            //店铺banner
            var html = template.render('distri_banner_tpl', data);
...
function get_list(change_param, is_init){
    ...
    load_class_storegoodslist.loadInit({
        'url':ApiUrl + '/index.php?act=distribute&op=distri_goods_list',
        'getparam':param,
