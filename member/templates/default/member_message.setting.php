...
<div class="wrap">
  ...
  <?php if (C('weixin_mp_isuse')) {?>
  <div class="alert alert-block">
    <ul>
        <li>关注公众号后，可以在微信客户端收到相关通知（不支持红包、代金券、到期提醒类消息）。</li>
        <?php if ($output['mp_openid']){?>
        <li>已经关注公众号，如果不想继续接收通知,“
            <a class="ncbtn ncbtn-aqua" style="color: #fff;" href="javascript:;void(0);" onclick="ajax_get_confirm('确认取消？', '<?php echo MEMBER_SITE_URL;?>/index.php?act=member_message&op=reset_openid');">
                取消</a>”账号的关联。</li>
        <?php }?>
    </ul>
  </div>
  ...
  <div class="ncm-message-setting">
    <form method="post" action="<?php echo urlMember('member_message', 'setting');?>" id="setting_form" onsubmit="ajaxpost('setting_form', '', '', 'onerror');">
      ...
      <?php if ($output['ticket']){?>
      <dl>
        <dt><span><i class="icon-qrcode"></i></span>微信通知</dt>
        <dd style="padding-top: 10px;">
          <img class="size100" alt="二维码" src="https://mp.weixin.qq.com/cgi-bin/showqrcode?ticket=<?php echo $output['ticket'];?>"> 扫码关注后，点击<a class="ncbtn ncbtn-aqua" href="javascript:window.location.reload();">刷新</a>重新加载。
        </dd>
      </dl>
      <?php }?>
      <dl>
...
<script type="text/javascript">
    <?php if ($output['ticket']){?>
        $(document).ready(function(){
            setTimeout("queryUserState()",5000);
        });
        
        function queryUserState(){
            $.ajax({
                type: "GET",
                url: "<?php echo MEMBER_SITE_URL?>/index.php?act=member_message&op=query_state",
                data: "",
                dataType: "json",
                timeout: 4000,
            ...
            setTimeout("queryUserState()",3000);
        }
    <?php }?>
</script> 
