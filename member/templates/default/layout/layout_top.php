        ...
        <div class="user-login-box" nctype="barLoginBox" style="display:none;"> <i class="arrow"></i> <a href="javascript:void(0);" class="close-a" nctype="close-barLoginBox" title="关闭">X</a>
          <form id="login_form" method="post" action="<?php echo urlLogin('login', 'login');?>" onsubmit="ajaxpost('login_form', '', '', 'onerror')">
            ...
            <div class="bottom">
              ...
              <a href="<?php echo MEMBER_SITE_URL;?>/index.php?act=connect_sina&ref_url=<?php echo urlencode(request_uri());?>" title="新浪微博账号登录" class="mr20">新浪微博</a>
              <?php } ?>
              <?php if (C('qq_isuse') == 1){?>
              <a href="<?php echo MEMBER_SITE_URL;?>/index.php?act=connect_qq&ref_url=<?php echo urlencode(request_uri());?>" title="QQ账号登录" class="mr20">QQ账号</a>
              <?php } ?>
            </div>
          </form>
        </div>
