...
<div class="wrap">
  ...
  <div class="ncm-security-container">
    ...
    <dl id="paypwd" class="<?php echo $output['member_info']['id_card_bind'] == 1  ? 'yes' : 'no';?>">
      <dt><span class="icon"><i></i></span><span class="item">
        <h4>实名认证</h4>
        <h6><?php echo $output['member_info']['id_card_bind'] == 1 ? '已设置' : '未设置';?></h6>
        </span></dt>
      <dd><span class="explain">实名认证通过后，不可修改；认证未通过，根据原因重新认证。
          <?php if($output['member_info']['id_card_state'] == 3){ ?>
          <p style="padding: 5px 0;color: #F33;">原因：<?php echo $output['member_info']['id_card_explain'];?></p>
          <?php } ?>
          <?php if($output['member_info']['id_card_state'] == 2){ ?>
            <p style="padding: 5px 0;">您认证的实名信息：<?php echo $output['real_name'];?>&nbsp;&nbsp;<?php echo substr_replace($output['member_info']['id_card_code'],'************',2,12);?></p>
          <?php } ?>
        </span>
        <span class="handle"><a href="index.php?act=member_security&op=auth&type=certification" class="ncbtn ncbtn-aqua bd">实名认证</a></span></dd>
    </dl>
  </div>
</div>
