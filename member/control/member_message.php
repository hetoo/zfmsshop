...
class member_messageControl extends BaseMemberControl {
    ...
    public function settingOp() {
        ...
        if (!empty($access_token)) {
            ...
            $mp_openid = $member_info['weixin_mp_openid'];
            if (!empty($mp_openid)) {
                $user_info = $logic_wx_api->getUserInfo($access_token,$mp_openid);//验证关注状态
                $mp_openid = $user_info['openid'];
            }
            Tpl::output('mp_openid', $mp_openid);
            if (empty($mp_openid)) {
                $_info = $logic_wx_api->getQrcode($access_token,$member_info['member_id']);
                Tpl::output('ticket', urlencode($_info['ticket']));//凭借此ticket可以在有效时间内换取二维码
            }
        }
     ...
     *
     */
    public function reset_openidOp() {
        $state = Model('member')->editMember(array('member_id' => $_SESSION['member_id']),array('weixin_mp_openid'=> ''));
        if ($state) {
