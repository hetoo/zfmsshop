...
class member_voucherControl extends BaseMemberControl
{
        ...
        if (! empty($list)) {
            foreach ($list as $k => $v) {
                if ($v['voucher_state'] == 2 && empty($v['voucher_order_id'])) {
                    ...
                    $voucher_order[$v['voucher_code']] = $k;
                }
            }
        }
        if (! empty($used_voucher_code)) {
            ...
            if (! empty($order_list)) {
                foreach ($order_list as $v) {
                    $k = $voucher_order[$v['voucher_code']];
                    $list[$k]['voucher_order_id'] = $v['order_id'];
     ...
     * 获取代金券模版详细信息
     */
    public function chain_voucher_listOp()
    {
        ...
        Tpl::output('voucherstate_arr', $model->getVoucherStateArray());
        Tpl::output('show_page', $model->showpage(2));
        $this->profile_menu('chain_voucher_list');
        Tpl::showpage('member_chain_voucher.list');
    }
    ...
    private function profile_menu($menu_key = '')
    {
        $menu_array = array(
            ...
            2 => array(
                'menu_key' => 'chain_voucher_list',
                'menu_name' => '门店代金券',
                'menu_url' => 'index.php?act=member_voucher&op=chain_voucher_list'
            ),
            3 => array(
