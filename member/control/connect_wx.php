...
class connect_wxControl extends BaseLoginControl{
    ...
    public function edit_infoOp(){
        if (chksubmit()) {
            ...
            $_url = urlMember('member', 'home');
            $state = $_POST['state'];
            if(!empty($state) && $state != 'js') $_url = base64_decode($state);
            showDialog(Language::get('nc_common_save_succ'),$_url,'succ');
        }
    }
    /**
     * 回调获取信息
        ...
        if(!empty($code)) {
            ...
            if(!empty($user_info['unionid'])) {
                ...
                if(!empty($member)) {//会员信息存在时自动登录
                    ...
                    if(!empty($state)) {
                        $_url = 'reload';
                        if($state != 'js') $_url = base64_decode($state);
                    }
                    showDialog('登录成功',$_url,'succ');
                }
    ...
    public function register($user_info){
        ...
        if(!empty($unionid)) {
            ...
            if(!empty($member)) {
                ...
                if(!empty($state)) {
                    $_url = 'reload';
                    if($state == 'js') showDialog('登录成功',$_url,'succ');
