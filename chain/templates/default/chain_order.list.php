...
<div class="alert alert-block mt10">
  <ul class="mt5">
    <li>该列表只可以查看待发货和已经发货的门店配送类型订单。</li>
  </ul>
</div>
<form method="get" action="index.php" target="_self">
  <table class="search-form">
    <input type="hidden" name="act" value="chain_order" />
    <input type="hidden" name="op" value="index" />
    <tr>
      <td>&nbsp;</td>
      <th style="width: 100px;">门店配送状态</th>
      <td class="w100"><select name="search_state_type">
          <option value="no" <?php if ($_GET['search_state_type'] == 'no') {?>selected<?php }?>>待配送</option>
          <option value="yes" <?php if ($_GET['search_state_type'] == 'yes') {?>selected<?php }?>>已完成</option>
        </select></td>
      <th> 订单编号
      </th>
  ...
  <tbody>
    ...
    <td class="bdl"></td>
      ...
      <td rowspan="<?php echo $_cont;?>" class="bdl"><?php echo $order_info['state_desc']; ?>
                ...
                <p><a href="javascript:void(0);" class="link" onclick="javascript:ajax_form('get_express', '查看物流', 'index.php?act=chain_order&op=get_express&shipping_express_id=<?php echo $order_info['extend_order_common']['shipping_express_id'];?>&shipping_code=<?php echo $order_info['shipping_code'];?>')">查看物流</a></p>
                <?php }?>
        </td>
      <td rowspan="<?php echo $_cont;?>" class="nscs-table-handle bdl bdr">
        <?php if ($order_info['order_state'] != ORDER_STATE_SUCCESS) { ?>
        <span><a href="javascript:void(0);" class="btn-bluejeans" onclick="javascript:ajax_form('chain_order', '发货', 'index.php?act=chain_order&op=send&order_id=<?php echo $order_info['order_id'];?>')">
        <p>发货</p>
        </a></span>
        <?php if ($order_info['order_state'] == ORDER_STATE_SEND) { ?>
        <span><a href="javascript:void(0);" class="btn-bluejeans" onclick="javascript:ajax_get_confirm('请确认已经完成配送（以免引起纠纷）？','<?php echo CHAIN_SITE_URL;?>/index.php?act=chain_order&op=succ_yes&order_id=<?php echo $order_info['order_id'];?>');">
        <p>配送成功</p>
        </a></span>
        <?php } ?>
        <?php } ?></td>
