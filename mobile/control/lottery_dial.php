...
class lottery_dialControl extends mobileHomeControl{
    private $member_id;
    function __construct() {
        parent::__construct();
        $this->member_id = parent::getMemberIdIfExists();
        if(!$this->member_id && $_POST['op'] == 'dial_prize'){
    ...
    public function dial_prizeOp(){
        ...
        $qua_condition['member_id'] = $this->member_id;
        $qua_condition['lot_type'] = 'dial';
        $qua_info = $dial_model->getOneDialQualifications($this->member_id, $qua_condition);
    ...
    private function _addPrizeInfo($dial_info, $lot_info){
        ...
        if(!$res){
            ...
            $dial_model->editDialQualifications($this->member_id, $condition, $update_data);
        }
    }
