...
class member_cartControl extends mobileMemberControl {
    ...
    public function cart_listOp() {
        ...
        $condition = array('buyer_id' => $this->member_info['member_id'], 'bl_id' => 0);
        $cart_list  = $model_cart->listCart('db', $condition);

        // 购物车列表 [得到最新商品属性及促销信息]
        $logic_buy_1 = logic('buy_1');
        $logic_buy_1->member_info = $this->member_info;
    ...
    public function cart_addOp() {
        ...
        if($goods_info['promotion_info']['promotion_type']==3) {
            $_n = Model('order')->getSpikeOrderLimit($this->member_info['member_id'],$goods_info);
            if($_n > 0) {
                output_error('抱歉，秒杀商品只能购买'.$_n.'次，请重新选择',array('spike'=>'1'));
            }
        }
        ...
        if($result) {
            ...
            if($dis_id && $goods_info['is_dis']) {//分销
    ...
    public function cart_edit_quantityOp() {
        ...
        if ($update) {
            ...
            $return['goods_price'] = ncPriceFormat($goods_info['goods_price']);
            $return['total_price'] = ncPriceFormat($goods_info['goods_price'] * $quantity);
            output_data($return);
        } else {
            output_error('修改失败');
        }
    ...
    private function _check_goods_storage(& $cart_info, $quantity, $member_id) {
        ...
        if ($cart_info['bl_id'] == '0') {
            ...
            //限时折扣
            $logic_buy_1->getPromotionInfo($goods_info,$quantity);
