...
class seller_goodsControl extends mobileSellerControl {
    ...
    public function goods_listOp() {
        ...
        switch ($goods_type) {
            case 'lockup':
                ...
                break;
            case 'wait_verify':
                $goods_list = $model_goods->getGoodsCommonWaitVerifyList($condition, $fields, $this->page);
                break;
            case 'offline':
        ...
        foreach ($goods_list as $key => $value) {
            $goods_list[$key]['goods_id'] = $storage_array[$value['goods_commonid']]['goods_id'];
    ...
    public function goods_infoOp() {
        ...
        if (empty($goodscommon_info) || $goodscommon_info['store_id'] != $this->store_info['store_id'] || ($_POST['type'] != 'app' && $goodscommon_info['goods_lock'] == 1)) {
            output_error('参数错误');
        }
    ...
    public function goods_showOp() {
        ...
        if (!is_array($_POST['commonids'])) $_POST['commonids'] = explode(',', rtrim($_POST['commonids'],','));
