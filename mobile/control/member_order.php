...
class member_orderControl extends mobileMemberControl {
    ...
    public function order_listOp() {
        ...
        $condition['order_type'] = array('in',array(1,3,4,5));
        ...
        foreach ($order_list_array as $value) {
            ...
            if ($value['order_state'] == ORDER_STATE_NEW) {
                if ($value['order_type'] != 2 && $value['payment_code'] == 'online') $order_group_list[$value['pay_sn']]['pay_amount'] += $value['order_amount'] - $value['rcb_amount'] - $value['pd_amount'];
...
//         $data['refund_state'] = $result['data']['order_info']['refund_state'];
        ...
        if(empty($data['reciver_phone'])) $data['reciver_phone'] = $result['data']['order_info']['extend_order_common']['reciver_info']['phone'];
        ...
        $data['store_default_im'] = $result['data']['order_info']['extend_store']['default_im'];
        $data['store_phone'] = $result['data']['order_info']['extend_store']['store_phone'];
        $data['chain_code'] = (($result['data']['order_info']['order_state'] == ORDER_STATE_NEW || $result['data']['order_info']['order_state'] == ORDER_STATE_PAY) && $result['data']['order_info']['chain_code']) ? $result['data']['order_info']['chain_code'] : '';
        $data['order_tips'] = $result['data']['order_info']['order_state'] == ORDER_STATE_NEW ? ($result['data']['order_info']['payment_code'] == 'chain' ? '请于'.CHAIN_ORDER_PAYPUT_DAY.'日内到店支付，逾期未支付订单将被关闭' : '请于'.ORDER_AUTO_CANCEL_TIME.'小时内完成付款，逾期未付订单自动关闭') : '';
