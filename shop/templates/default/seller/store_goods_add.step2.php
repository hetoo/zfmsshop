...
var BATCH_FLAG = <?php if($output['edit_goods_sign'] && $output['goods']['is_batch'] == 1){?>true<?php }else{?>false<?php }?>;
</script>
...
#saleRulePanel a:hover{  color: #FFF;}
.trade-setting-box { display: inline-block; vertical-align: top;}
.trade-setting-box .tit-bar { background-color: #D9EDF7; border: solid #BCE8F1; border-width: 1px;}
.trade-setting-box .tit-bar .quantity { display: inline-block; padding: 5px 100px 5px 70px; font-weight: 600; line-height: 24px;}
.trade-setting-box .tit-bar .unit-price { display: inline-block; padding: 5px 0 5px 10px; font-weight: 600; line-height: 24px;}
.trade-setting-box .con-row { display: block; padding: 5px 8px 5px 0; border: solid #BCE8F1; border-width: 0 1px 1px 1px;}
.trade-setting-box .con-row div { position: relative; display: inline;}
.trade-setting-box .con-row span { display: inline-block; vertical-align: top; padding: 4px 10px; line-height: 20px;}
.trade-setting-box .con-row span a { vertical-align: top;}
.trade-setting-box .con-row .text { width: 85px; vertical-align: top;}
.m-t-10 { margin-top: 10px !important;}
.btn-default { background-color: #B6C2C9; border-color: #B6C2C9; color: #FFF;}
.btn-sm{ border-radius: 3px !important; font-size: 12px !important; line-height: 1.5 !important; padding: 5px 10px !important;}
.btn-danger { background-color: #FF5B57; border-color: #FF5B57; color: #FFF;}
.btn-xs{ border-radius: 3px !important; font-size: 12px !important; line-height: 1.5 !important; padding: 1px 5px !important;}
</style>
...
<div class="item-publish">
  <form method="post" enctype="multipart/form-data" id="goods_form" action="<?php if ($output['edit_goods_sign']) { echo urlShop('store_goods_online', 'edit_save_goods');} else { echo urlShop('store_goods_add', 'save_goods');}?>">
    ...
    <div class="ncsc-form-goods">
      ...
      <?php if (!$output['edit_goods_sign']){?>
      <dl class="goods_sel_type">
        <dt>商品类型<?php echo $lang['nc_colon'];?></dt>
        <dd>
          <input type="radio" name="goods_sel_type" value="single" checked="checked">&nbsp;零售商品
          &nbsp;&nbsp;&nbsp;&nbsp;
          <input type="radio" name="goods_sel_type" value="batch">&nbsp;批发商品
        </dd>
      </dl>
      <?php }else{?>
      <input type="hidden" name="goods_sel_type" value="<?php echo $output['goods']['is_batch'] == 1?'batch':'single';?>">
      <?php }?>
      <?php  if (!$output['edit_goods_sign'] || $output['edit_goods_sign'] && $output['goods']['is_batch'] == 1){?>
      <dl class="spec-bg" id="saleRulePanel" <?php echo !$output['edit_goods_sign']?'style="display: none;"':'';?>>
        <dt><i class="required">*</i>销售规则：</dt>
        <dd id="goodsModalM">
          <div class="trade-setting-box" id="tradeSettingBox">
            <div class="tit-bar">
              <span class="quantity"><i class="required">*</i>购买数量</span>
              <span class="unit-price"><i class="required">*</i>商品单价</span>
            </div>
            <?php 
              $batch_list_count = count($output['goods']['batch_price']);
              if($batch_list_count > 0 && $output['edit_goods_sign']){
                $_i_batch = 1;
                foreach ($output['goods']['batch_price'] as $k => $v) {                 
            ?>
            <div class="con-row" data-index="<?php echo $_i_batch;?>">
              <div class="batch-num">
                <span>购买量</span>
                <input class="text" value="<?php echo $k;?>" type="text" <?php echo $_i_batch == 1?' name="batch_num" readonly="readonly" style="background:#E7E7E7 none"':' name="_batchNum[]"'?> >
              </div>
              <div class="batch-price">
                <span>及以上：</span>
                <input class="text" type="text" <?php  echo $_i_batch == 1?' name="batch_price" id="batch_price" ':' name="_batchPrice[]"';?> value="<?php echo $v;?>">
                <em class="add-on">元</em>
                <?php if($batch_list_count == $_i_batch && $batch_list_count > 1){?>
                <span><a href="javascript:;" data-del="<?php echo $_i_batch;?>" class="btn btn-xs btn-danger">删除</a></span>
                <?php }?>
              </div>
            </div>
            <?php $_i_batch++;}?>
            <?php }else{?>
            <div class="con-row" data-index="1">
              <div class="batch-num">
                <span>购买量</span>
                <input class="text" value="1" name="batch_num" type="text" readonly="readonly" style="background:#E7E7E7 none">
              </div>
              <div class="batch-price">
                <span>及以上：</span>
                <input class="text" name="batch_price" type="text" id="batch_price">
                <em class="add-on">元</em>
              </div>
            </div>
            <?php }?>
            <div class="m-t-10">
              <a href="javascript:;" class="ncbtn btn-sm btn-default" id="addPriceRange">
                <i class="fa icon-plus" aria-hidden="true"></i>添加价格区间 
              </a>
              <span></span>
            </div>
          </div>          
          <p class="hint"> 该项设置用于设定商品最低起购量及更多起购量时所对应的销售价格，最多不超过5个起购点，即形成5段阶梯价格。默认购买数量与商品单价为必填选项。<br>
            <strong>操作技巧：</strong>阶梯起定价应为购买数量与价格成反比，即购买数量越大价格越低。</p>
        </dd>
      </dl>
      <?php }?>
      <?php  if (!$output['edit_goods_sign'] || $output['edit_goods_sign'] && $output['goods']['is_batch'] == 0){?>
      <dl class="saleRuleSingle">
      ...
      <dl nctype="spec_group_dl" class="spec-bg" <?php if($k == '1'){?>spec_img="t"<?php }?>>
        <dt>
          ...
          <table border="0" cellpadding="0" cellspacing="0" class="spec_table">
            ...
            <th nctype="spec_name_<?php echo $k;?>"><?php if (isset($output['goods']['spec_name'][$k])) { echo $output['goods']['spec_name'][$k];} else {echo $val['sp_name'];}?></th>
              ...
              <th class="w90"><span class="red">*</span><?php echo $lang['store_goods_index_price'];?>
                <div class="batch"><i class="icon-edit" title="批量操作" id="b_price"></i>
                  <div class="batch-input" style="display:none;">
                    <h6>批量设置价格：</h6>
                    <a href="javascript:void(0)" class="close">X</a>
                    <input name="" type="text" class="text price" />
      ...
      <dl>
        <dt><?php echo '商品视频'.$lang['nc_colon'];?></dt>
        <dd>
          <div class="ncsc-goods-default-video">
            <div class="goodsvideo-uplaod">
              <div class="upload-thumb">
                <?php if($output['edit_goods_sign']) {?>
                    <?php if(empty($output['goods']['goods_video']) && !file_exists(BASE_UPLOAD_PATH . '/' . ATTACH_GOODS . '/' . $output['goods']['store_id'] . '/' . 'goods_video' . '/' . $output['goods']['goods_video'])){?>
                      <video poster="<?php echo goodsVideoPath($output['goods']['goods_video'] , $output['goods']['store_id'])?>" nctype="goods_video" src=""></video>
                    <?php }else{ ?>
                      <video poster="" nctype="goods_video" src="<?php echo goodsVideoPath($output['goods']['goods_video'] , $output['goods']['store_id'])?>"></video>
                    <?php }?>
                <?php }else{ ?>
                  <video poster="<?php echo goodsVideoPath($output['goods']['goods_video'] , $output['goods']['store_id'])?>" nctype="goods_video" src=""></video>
                <?php }?>
              ...
              <div class="handle">
                <div class="ncsc-upload-btn">
                  ...
                  <input type="file" hidefocus="true" size="1" class="input-file" name="goods_video" id="goods_video">
                  </span>
                    <p><i class="icon-upload-alt"></i>视频上传</p>
                  </a> </div>
                <a class="ncbtn mt5" nctype="show_video" href="<?php echo urlShop('store_video', 'video_list', array('item'=>'goods'));?>"><i class="icon-picture"></i>从视频空间选择</a>
                <a href="javascript:void(0);" nctype="del_goods_video_demo" class="ncbtn mt5" style="display: none;"><i class="icon-circle-arrow-up"></i>关闭空间</a></div>
            </div>
          </div>
      ...
      <dl nctype="virtual_null" <?php if ($output['goods']['is_virtual'] == 1) {?>style="display:none;"<?php }?>>
        ...
        <dd>
          <ul class="ncsc-form-radio-list">
            <li>
              <label>
                <input name="g_inv" value="1" <?php if (!empty($output['goods']) && $output['goods']['goods_inv'] == 1) { ?>checked="checked" <?php } ?> type="radio" />
                <?php echo $lang['nc_yes'];?></label>
                <div><label><input class="vm" type="checkbox" value="1" name="g_vat" <?php if (!empty($output['goods']) && $output['goods']['goods_vat'] == 1) { ?>checked="checked" <?php } ?>>
                增值税发票</label></div>
            </li>
            <li>
              <label>
                <input name="g_inv" value="0" <?php if (empty($output['goods']) || $output['goods']['goods_inv'] == 0) { ?>checked="checked" <?php } ?> type="radio"/>
...
$(function(){
    ...
    $.validator.addMethod('checkPrice', function(value,element){
      ...
      if(BATCH_FLAG){
        _g_price = parseFloat($('input[name="batch_price"]').val());
      }
    ...
    $('#goods_form').validate({
        errorPlacement: function(error, element){
            ...
            if(element_name == 'batch_price'){
              $(element).parents('#tradeSettingBox').find('#addPriceRange').nextAll('span').append(error);
            }else{
        ...
        rules : {
            ...
            g_price : {
                required    : function(){ return !BATCH_FLAG;},
                number      : true,
                min         : function(){ return !BATCH_FLAG ? 0.01 : 0;},
                max         : 9999999,
                checkPrice  : function(){ return !BATCH_FLAG;}
            },
            batch_price : {
                required    : function(){ return BATCH_FLAG;},
                number      : true,
        ...
        messages : {
            ...
            g_price : {
                required    : '<i class="icon-exclamation-sign"></i><?php echo $lang['store_goods_index_store_price_null'];?>1',
                number      : '<i class="icon-exclamation-sign"></i><?php echo $lang['store_goods_index_store_price_error'];?>2',
                min         : '<i class="icon-exclamation-sign"></i><?php echo $lang['store_goods_index_store_price_interval'];?>3',
                max         : '<i class="icon-exclamation-sign"></i><?php echo $lang['store_goods_index_store_price_interval'];?>4',
                checkPrice  : '<i class="icon-exclamation-sign"></i>商品价格不能高于市场价格'
            },
            batch_price : {
                required    : '<i class="icon-exclamation-sign"></i><?php echo '第一阶梯价不能为空';?>',
...
$(function(){
	...
	$('input[type="submit"]').click(function(){
        ...
        if(BATCH_FLAG){
          $(this).val($('input[name="batch_price"]').val());
        }
			}
		});
...
function into_array(){
    var sku_list = {};
    str = '';
    if ($("dl[nctype=spec_group_dl]").find('input[type="checkbox"]:checked').size()) {
        sku_list['i_'] = '';
        $("dl[nctype=spec_group_dl]").each(function(){
            var _color = '';
            if ( $(this).attr('spec_img') ) _color = '1';
    		var obj_list = $(this).find('input[type="checkbox"]:checked');
    		if ( obj_list.size() == 0 ) {
    		...
    		if ( obj_list.size() > 0 ) {
    		    ...
    		    obj_list.each(function(){
        			...
        			_v = $(this).val();
        			$.each(sku_list, function(k, v){
        			    _list[''+k+i] = ''+v+'<td s_id="'+i+'" c_id="'+_color+'">'+_v+'</td>';
        			});
    		    });
    		    sku_list = _list;
    		}
        ...
        $.each(sku_list, function(spec_bunch, v){
            str += '<tr sku_id="'+spec_bunch+'"><input type="hidden" name="spec['+spec_bunch+'][goods_id]" nc_type="'+spec_bunch+'|id" value="" />'+v;
...
function update_sku(){
    $("tr[sku_id]").each(function(){
        var _k = $(this).attr('sku_id');
        $(this).find('td[s_id]').each(function(){
            var _id = $(this).attr('s_id');
            var _c = $(this).attr('c_id');
            var _s = $(this).html();
            _s += '<input type="hidden" name="spec['+_k+'][sp_value]['+_id+']" value="'+_s+'" />';
            if ( _c ) _s += '<input type="hidden" name="spec['+_k+'][color]" value="'+_id+'" />';
            $(this).html(_s);
        });
    });
}
...
function goods_stock_set(){
    ...
    if(str == ''){
        ...
        update_sku();
        goods_sku_price_limit(BATCH_FLAG);
    }
    $('div[nctype="spec_div"]').perfectScrollbar('update');
}

//设置sku价格修个限制
function goods_sku_price_limit(is_batch_flag){
  if(is_batch_flag){
    $('tbody[nc_type="spec_table"]').find('input[data_type="price"]').attr('readonly','readonly').css('background','#E7E7E7 none');
    $('tbody[nc_type="spec_table"]').find('input[data_type="price"]').val($('#batch_price').val());
    $('#b_price').hide();
  }else{
    $('tbody[nc_type="spec_table"]').find('input[data_type="price"]').removeAttr('readonly');
    $('tbody[nc_type="spec_table"]').find('input[data_type="price"]').css('background','none');
    $('#b_price').show();
  }
}
...
$('.goods_sel_type input[type="radio"]').on('click',function(){
  ...
  if(cur_val == 'batch'){
    ...
    $('#saleRulePanel').hide();
    $('.saleRuleSingle').show();    
    $('input[name="g_price"]').val($('#batch_price').val());
  }
  goods_sku_price_limit(BATCH_FLAG);
});

$('#addPriceRange').click(function(){
      var row_index = parseInt($('#saleRulePanel .con-row').size());
      if(row_index >= 5) return false;
      var batch_index = parseInt($('#saleRulePanel .batch-num:last').find('input').val());
      var row_count = row_index+1;
      var batch_num = batch_index+1;
      var batch_html = '<div class="con-row" data-index="'+row_count+'"><div class="batch-num"><span>购买量</span>';
      batch_html += '&nbsp;<input class="text" value="'+batch_num+'" name="_batchNum[]" type="text">';
      batch_html += '</div><div class="batch-price"><span>&nbsp;及以上：</span>';
      batch_html += '&nbsp;<input class="text" name="_batchPrice[]" type="text">';
      batch_html += '&nbsp;<em class="add-on">元</em></div>';
      batch_html += '<span><a href="javascript:;" data-del="'+row_count+'" class="btn btn-xs btn-danger">删除</a></span>';
      $('.btn-xs').parent('span').remove();
      $(this).parent('div').before(batch_html);
});
$('#saleRulePanel').on('click','.btn-xs',function(){
      var row_count = parseInt($(this).attr('data-del'));          
      var pre_num = row_count - 1;
      $('div[data-index='+row_count+']').remove();
      if(row_count > 2){
        var batch_html = '<span><a href="javascript:;" data-del="'+pre_num+'" class="btn btn-xs btn-danger">删除</a></span>';
        $('div[data-index='+pre_num+']').append(batch_html);
      }else{
        return false;
      }
}); 
<?php if(is_array($output['spec_list']) && !empty($output['spec_list'])){?>
  $('#batch_price').on('change',function(){
    $('tbody[nc_type="spec_table"]').find('input[data_type="price"]').val($(this).val());
  });
<?php }?>
$('#goodsModalM').on('change','.batch-price > input',function(){
  var cur_val = parseInt($(this).val());
  var pre_obj = $(this).parents('div.con-row').prev('div.con-row').find('.batch-price > input');
  var pre_val = parseInt(pre_obj.val());
  var next_obj = $(this).parents('div.con-row').next('div.con-row').find('.batch-price > input');
  var next_val = parseInt(next_obj.val());
  if(cur_val >= pre_val){
    cur_val = pre_val-1;
    $(this).val(cur_val).change();
  }
  if(cur_val <= next_val){
    next_obj.val(cur_val-1).change();
  }
});
$('#saleRulePanel').on('change','.batch-num > input',function(){
  var cur_val = parseInt($(this).val());
  var pre_obj = $(this).parents('div.con-row').prev('div.con-row').find('.batch-num > input');
  var pre_val = parseInt(pre_obj.val());
  var next_obj = $(this).parents('div.con-row').next('div.con-row').find('.batch-num > input');
  var next_val = parseInt(next_obj.val());
  if(cur_val <= pre_val){
    cur_val = pre_val+1;
    $(this).val(cur_val).change();
