...
<div class="ncsc-form-default">
  <form method="post" action="<?php echo empty($output['chain_info']) ? urlShop('store_chain', 'chain_add') : urlShop('store_chain', 'chain_edit', array('chain_id' => $output['chain_info']['chain_id']));?>" id="chain_form" enctype="multipart/form-data">
    ...
    <dl>
      <dt><i class="required">*</i>结算周期<?php echo $lang['nc_colon'];?></dt>
      <dd>
        <input type="text" class="text w200" name="chain_cycle" id="chain_cycle" value="<?php echo $output['chain_info']['chain_cycle'];?>" />&nbsp;天
        <p class="hint">请认真填写您与门店的结算周期，以便系统为您生成该门店结算账单。</p>
      </dd>
    </dl>
    <dl>
    ...
    <dl>
      <dt><i class="required">*</i>门店LOGO<?php echo $lang['nc_colon'];?></dt>
      <dd>
        <?php if ($output['chain_info']['chain_logo'] != '') {?><p><img src="<?php echo getChainImage($output['chain_info']['chain_logo'], $_SESSION['store_id']);?>" style="max-width: 640px; max-height: 240px;" /></p>
        <?php }?>
        <p>
          <input type="file" hidefocus="true" name="chain_logo" id="chain_logo" />
        </p>
        <p class="hint">请上传门店LOGO，建议横幅图片尺寸为120px*80px</p>
      </dd>
    </dl>
    <dl>
    ...
    <dl>
      <dt><i class="required">*</i>门店横幅<?php echo $lang['nc_colon'];?></dt>
      <dd>
        <?php if ($output['chain_info']['chain_banner'] != '') {?><p><img src="<?php echo getChainImage($output['chain_info']['chain_banner'], $_SESSION['store_id']);?>" style="max-width: 640px; max-height: 240px;" /></p>
        <?php }?>
        <p>
          <input type="file" hidefocus="true" name="chain_banner" id="chain_banner" />
        </p>
        <p class="hint">请上传门店横幅，建议横幅图片尺寸为1200px*350px</p>
      </dd>
    </dl>
    <div class="bottom">
...
$(function(){
    ...
    $('#chain_form').validate({
        ...
        messages : {
            ...
            chain_logo : {
                required : '<i class="icon-exclamation-sign"></i>请上传门店LOGO'
            },
            chain_img : {
                required : '<i class="icon-exclamation-sign"></i>请上传实拍图片'
            },
            chain_banner : {
                required : '<i class="icon-exclamation-sign"></i>请上传门店横幅图片'
            }
        }
    });
});
