...
<ul class="goods-list">
  ...
  <li>
    ...
    <a nctype="btn_add_flash_goods" data-storage="<?php echo $val['goods_storage'];?>" data-goods-id="<?php echo $val['goods_id'];?>" data-goods-name="<?php echo $val['goods_name'];?>" data-goods-img="<?php echo thumb($val, 240);?>" data-goods-price="<?php echo $val['goods_price'];?>" href="javascript:void(0);" class="ncbtn-mini">选择商品/修改闪购价</a>
    <?php } else { ?>
...
<?php } ?>
<div id="dialog_add_flash_goods" style="display:none;">
  <input id="dialog_goods_id" type="hidden">
  <input id="dialog_input_goods_price" type="hidden">
  <div class="eject_con">
    <div id="dialog_add_flash_goods_error" class="alert alert-error">
      <label for="dialog_flash_price" class="error" ><i class='icon-exclamation-sign'></i>闪购价格不能为空，且必须小于商品价格</label>
    </div>
    ...
    <dl>
      <dt>闪购价格：</dt>
      <dd>
        <input id="dialog_flash_price" type="text" class="text w70">
        <em class="add-on"><i class="icon-renminbi"></i></em>
        <p class="hint">闪购价应低于正常商品售价，活动开始时，系统将自动转换销售价为促销价。</p>
      </dd>
    </dl>
    <div class="eject_con">
      <div class="bottom">
