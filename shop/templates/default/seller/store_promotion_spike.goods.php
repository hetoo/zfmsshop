...
<ul class="goods-list">
  ...
  <li>
    ...
    <?php if(empty($output['p_list'][$val['goods_id']])){?>
    <a nctype="btn_add_spike_goods" data-storage="<?php echo $val['goods_storage'];?>" data-goods-id="<?php echo $val['goods_id'];?>" data-goods-name="<?php echo $val['goods_name'];?>" data-goods-img="<?php echo thumb($val, 240);?>" data-goods-price="<?php echo $val['goods_price'];?>" href="javascript:void(0);" class="ncbtn-mini">选择商品/修改秒杀价</a>
    <?php } else { ?>
    <a href="javascript:void(0);" class="ncbtn-mini">已参加同时段促销</a>
    <?php } ?>
     </li>
  <?php } ?>
</ul>
...
<?php } ?>
<div id="dialog_add_spike_goods" style="display:none;">
  <input id="dialog_goods_id" type="hidden">
  <input id="dialog_input_goods_price" type="hidden">
  <div class="eject_con">
    <div id="dialog_add_spike_goods_error" class="alert alert-error">
      <label for="dialog_spike_price" class="error" ><i class='icon-exclamation-sign'></i>秒杀价格不能为空，且必须小于商品价格</label>
    </div>
    ...
    <dl>
      <dt>秒杀价格：</dt>
      <dd>
        <input id="dialog_spike_price" type="text" class="text w70">
        <em class="add-on"><i class="icon-renminbi"></i></em>
        <p class="hint">秒杀价应低于正常商品售价，活动开始时，系统将自动转换销售价为促销价。</p>
      </dd>
    </dl>
    <div class="eject_con">
      <div class="bottom">
