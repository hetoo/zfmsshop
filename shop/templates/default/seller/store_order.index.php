...
<table class="ncsc-default-table order">
  <thead>
    ...
    <tr>
      ...
      <td colspan="20"><label for="all" ><?php echo $lang['nc_select_all'];?></label>
        <a href="javascript:void(0);" class="ncbtn-mini" nctype="batch" data-param="{url:'<?php echo urlShop('store_deliver', 'batch_send');?>', sign:'send'}"><i></i>批量发货</a>
        <a href="javascript:void(0);" class="ncbtn-mini" nctype="batch" data-param="{url:'<?php echo urlShop('store_order', 'batch_order_print');?>', sign:'print'}"><i class="icon-print"></i>批量打印</a>
      </td>
    </tr>
    <?php }?>
  </thead>
  ...
  <tbody>
    ...
    <tr>
      <th colspan="20">
        <span style="margin: 0 0 0 5px;"><input type="checkbox" class="checkitem tc" value="<?php echo $order_id; ?>"/></span>
        <span class="ml10"><?php echo $lang['store_order_order_sn'].$lang['nc_colon'];?><em><?php echo $order['order_sn']; ?></em>
    ...
    <tr>
      ...
      <!-- 取消订单 -->
      <td class="bdl bdr" rowspan="<?php echo $order['goods_count'];?>"><?php if($order['if_store_cancel'] || $order['if_seller_refund_cancel']) { ?>
...
$(function(){
    ...
    $('a[nctype="batch"]').click(function(){
        ...
        if(data_str.sign == 'send'){
            window.location.href = data_str.url + '&order_id=' + _items;
        }else if(data_str.sign == 'print'){
            window.open(data_str.url + '&order_id=' + _items);
        }
    });
