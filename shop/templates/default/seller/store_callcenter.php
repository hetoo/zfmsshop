...
<div class="ncsc-form-default">
  ...
  <form method="post" action="index.php?act=store_callcenter&op=save" id="callcenter_form" onsubmit="ajaxpost('callcenter_form','','','onerror')" class="ncs-message">
    ...
    <dl>
      <dt>站内IM默认客服<?php echo $lang['nc_colon']?></dt>
      <dd>
        <div class="ncs-message-list">
            <span class="tool tip" title="默认的是开店会员帐号，可修改为其他客服">
          <select name="default_im">
    <?php if (is_array($output['seller_list']) && !empty($output['seller_list'])) { ?>
    <?php foreach ($output['seller_list'] as $key => $val) { ?>
        <option value="<?php echo $val['member_id'];?>" <?php if($val['member_id'] == $output['storeinfo']['default_im']){?>selected="selected"<?php }?>><?php echo $val['seller_name'];?></option>
    <?php } ?>
    <?php } ?>
          </select>
          </div>
