...
<div class="ncc-main">
  ...
  <form action="<?php echo urlShop('buy','buy_step1');?>" method="POST" id="form_buy" name="form_buy">
    <input type="hidden" value="1" name="ifcart" id="ifcart">
...
<?php } ?> /></td>
          ...
          <td><!-- S 商品单价 -->
            ...
            <?php if (!empty($cart_info['promotion_info'])) {?>
...
$(function() {
	$('a[nc_type="chain"]').on('click',function(){
		var chains = 0,store_id = $(this).attr('data_store_id');
		$('input[name="cart_id[]"]').each(function(){
    			if ($(this).attr('data_store_id') != store_id) {
    				$(this).attr('checked',false);
    			} else {
    				if ($(this).prop('checked') && $(this).attr('data_chain') == '1') {
    				    var cart_id = $(this).parent().parent().attr('nc_group');
            			$('#ifcart').val('');
            			$("input[goods_id][id!='cart_id"+cart_id+"']").attr('checked',false);
    				    $(this).val($(this).attr('goods_id')+'|'+$('#input_item_'+cart_id).val());
            			$('#ifchain').val('1');
            			$('#form_buy').submit();
            			chains = 1;
            			return false; 
...
window.jjgCallback = (function(jjgDetails) {
    ...
    var jjgHeader = function(id, selectedCost) {
        ...
        if (headers[id]) {
            ...
            $(":checkbox[data-jjg='"+id+"']:first").parents('tr.shop-list').before(s);
            $(":checkbox[data-jjg='"+id+"']:last").parents('tr.shop-list').after(jjgFooterTpl);
            var $tr = $(":checkbox[data-jjg='"+id+"']:first").parents('tr.shop-list').prev();
            var $tr2 = $(":checkbox[data-jjg='"+id+"']:last").parents('tr.shop-list').next();
