...
<div class="ncc-receipt-info">
  ...
  <div id="addr_list" class="ncc-candidate-items">
    <ul>
      <li><span class="true-name"><?php echo $output['address_info']['true_name'];?></span><span class="address"><?php echo $output['address_info']['type'].$output['address_info']['area_info'],$output['address_info']['address'];?></span><span class="phone"><i class="icon-mobile-phone"></i><?php echo $output['address_info']['mob_phone'] ? $output['address_info']['mob_phone'] : $output['address_info']['tel_phone'];?></span></li>
    </ul>
  </div>
</div>
...
function showStoreProduct(_chain) {
    ifchain = _chain;
    <?php if ($output['chain_store_id']) {?>
    var obj = {
        'ifchain': ifchain,
        'cart_id[]': chain_cart_id,
        'chain_id': chain_id,
        'shopnc_cart_list': 1
        };
	$.ajax({
		type: "POST",
		url: 'index.php?act=buy&op=buy_step1',
		data: obj,
		async: false,
	    success: function(rs){
	        $("tbody[store_id='"+chain_store_id+"']").remove();
	        $("#buy_store_cart_list").before(rs);
