...
<div class="ncc-main">
  <?php if ($output['order_type'] == 'pd_order') {?>
  <div class="ncc-title">
    <h3>支付提交</h3>
  ...
  <?php }else if($output['order_type'] == 'etms_order'){?>
  <div class="ncc-title">
    <h3>支付提交</h3>
  </div>
    <div class="ncc-receipt-info">
    <div>保证金支付单号 : <?php echo $output['etms_info']['etm_sn'];?></div>
      <div class="ncc-receipt-info-title">
        <h3>您已申请在线支付店铺入驻保证金，请立即在线支付！
          保证金金额：<strong>￥<?php echo $output['etms_info']['etm_amount'];?></strong> </h3>
      </div>
    </div>
  <?php }else if($output['order_type'] == 'etmc_order'){?>
  <div class="ncc-title">
    <h3>支付提交</h3>
    <h5>查看保证金支付记录可以通过<a href="<?php echo urlShop('earnest_money');?>" target="_blank">保证金记录 </a>进行查看。</h5>
  </div>
    <div class="ncc-receipt-info">
    <div>保证金支付单号 : <?php echo $output['etmc_info']['etm_sn'];?></div>
      <div class="ncc-receipt-info-title">
        <h3>您已申请在线支付门店开设保证金，请立即在线支付！
          保证金金额：<strong>￥<?php echo $output['etmc_info']['etm_amount'];?></strong> </h3>
      </div>
    </div>
  <?php } else {?>
  <div class="ncc-title">
        ...
        success: function(result) {
            if(result.state==1){
                if (result.type == 'r') {
                    ...
                    window.location.href = '<?php echo urlMember('predeposit');?>';
                } else {
