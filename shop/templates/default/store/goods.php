...
.ncs-meta .price-title{ width: 76px; }
.ncs-meta .price-title,.ncs-meta .price-list{ display: inline-block; }
.ncs-meta .price-list{ width: 430px; }
.ncs-meta .price-list div.price-info{ width: 85px; display: inline-block;}
.ncs-meta .price-list div.price-info dt{ text-align: left; width: 83px; margin-right: 2px;}
</style>
<div class="wrapper pr">
  ...
  <div class="ncs-detail<?php if ($output['store_info']['is_own_shop']) echo ' ownshop'; ?>"> 
    ...
    <div class="ncs-goods-summary">
      ...
      <?php if($output['goods']['is_batch']){?>
      <div class="ncs-meta02" >
        <!-- S 批发价 -->
        <dl>
          ...
          <div class="price-list">
            <?php if(!empty($output['batch_price'])){?>
            <?php foreach($output['batch_price'] as $key => $value){?>
            <div class="price-info">
              <dt>
                <span class="fd">¥</span>
                <span class="F-key"><?php echo ncPriceFormat($value['price']);?></span>
              </dt>
              <dt><?php echo $value['batch'];?></dt>
            </div>
            <?php }?>
            <?php }?>
          </div>
        ...
        <dl class="rate">
          <dt>商品评分：</dt>
          <!-- S 描述相符评分 -->
          <dd><span class="raty" data-score="<?php echo $output['goods_evaluate_info']['star_average'];?>"></span><a href="#ncGoodsRate">共有<?php echo $output['goods']['evaluation_count']; ?>条评价</a>(销量：<?php echo $output['goods']['goods_salenum']; ?>)</dd>
          <!-- E 描述相符评分 -->
        </dl>
        <!-- E 商品评论 -->
      </div>
      ...
      <div class="ncs-meta">
        ...
        <!-- S 商品参考价格 -->
        <dl>
          <dt><!--<?php echo $lang['goods_index_goods_cost_price'];?><?php echo $lang['nc_colon'];?>-->市场价</dt>
          <dd class="cost-price"><strong><?php echo $lang['currency'].ncPriceFormat($output['goods']['goods_marketprice']);?></strong></dd>
        </dl>
        <!-- E 商品参考价格 --> 
        <!-- S 商品发布价格 -->
        <dl>
          <dt><!--<?php echo $lang['goods_index_goods_price'];?><?php echo $lang['nc_colon'];?>-->商城价</dt>
          <dd class="price">
            <?php if (isset($output['goods']['promotion_price']) && !empty($output['goods']['promotion_price'])) {?>
            <strong><i><?php echo $lang['currency'];?></i><?php echo ncPriceFormat($output['goods']['promotion_price']);?></strong>
            <em>(<?php if($output['goods']['is_member_price'] == '1' && $output['goods']['member_price'] > 0 ){ ?>会员价，<?php }?>原售价<?php echo $lang['nc_colon'];?><?php echo $lang['currency'].ncPriceFormat($output['goods']['goods_price']);?>)</em>
            <?php } else {?>
                  <strong><?php echo $lang['currency'].ncPriceFormat($output['goods']['goods_price']);?></strong>
                  <?php if($_SESSION['is_login'] != '1' && $output['goods']['is_member_price'] == '1'){ ?>
                    <em><a href='javascript:void(0);' onclick="login_dialog();">登录</a>后确认是否享有会员价</em>
                  <?php }?>
            <?php }?>
          </dd>
        </dl>
      ...
      <?php if (isset($output['goods']['promotion_type']) || !empty($output['goods']['jjg_explain']) || !empty($output['mansong_info'])) {?>
        <dl>
          ...
          <dd class="promotion-info">
            ...
            <?php }?>
            <!-- E 限时折扣  --> 
            <?php if ($output['goods']['promotion_type'] == 'spike') {?>
            <span class="sale-rule w400">直降<em><?php echo $lang['currency'].ncPriceFormat($output['goods']['down_price']);?></em>
            <?php if($output['goods']['upper_limit']) {?>
            <?php echo sprintf('最多限购%s件，',$output['goods']['upper_limit']);?><?php echo $output['goods']['explain'];?>
            <?php } ?>
            </span>
            <?php }?>
            <?php if ($output['goods']['promotion_type'] == 'flash') {?>
            <span class="sale-rule w400">直降<em><?php echo $lang['currency'].ncPriceFormat($output['goods']['down_price']);?></em>
            <?php if($output['goods']['upper_limit']) {?>
            <?php echo sprintf('最多限购%s件，',$output['goods']['upper_limit']);?><?php echo $output['goods']['explain'];?>
            <?php } ?>
    ...
    <div class="tabbar pngFix" id="main-nav">
      <div class="ncs-goods-title-nav">
        <ul id="categorymenu">
          ...
          <span class="ncs-goods-code-note">客户端扫购有惊喜</span> </div></div>
        </ul>
...
<script>
  ...
  if (window.ActiveXObject) {
    ...
    if (Sys.ie <= 8) {
      var videoObj = $('.bigImg video');
      if(videoObj != ""){
        $('.bigImg').find("video").remove();
        $('.bigImg').find("img").css("display" , "block");
        $('.list-video-img').remove();
        $('.list-img').addClass("list-img-hover");
      }
    }
  }
