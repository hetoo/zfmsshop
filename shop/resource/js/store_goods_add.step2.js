...
$(function(){
    ...
    $('#goods_video').fileupload({
        dataType: 'json',
        url: SITEURL + '/index.php?act=store_goods_add&op=video_upload&upload_type=uploadedfile',
        formData: {name:'goods_video'},
        add: function (e,data) {
            $('video[nctype="goods_video"]').attr('poster', SHOP_TEMPLATES_URL + '/images/loading.gif');
    ...
    $('input[name="g_vat"]').click(function(){//增值税发票
        if($(this).attr('checked')){
            $('[name="g_inv"]').first().click();
        }
    });
    $('input[name="g_inv"]').last().click(function(){//不开发票
        $('[name="g_vat"]').removeAttr("checked");
    });

    // 商品视频显示隐藏
    $('input[nctype="video"]').click(function(){
        $('input[nctype="video"]').nextAll('div[nctype="div_video"]').hide();
        $(this).nextAll('div[nctype="div_video"]').show();
    });
    ...
    // 计算折扣
    $('input[name="g_price"],input[name="batch_price"],input[name="g_marketprice"]').change(function(){
        discountCalculator();
    });
    ...
    $('[name="is_gv"]').change(function(){
        if ($('#is_gv_1').prop("checked")) {
            ...
            $('[name="goods_sel_type"]').first().click();
        } else {
...
function discountCalculator() {
    ...
    if(BATCH_FLAG){
        _price = parseFloat($('input[name="batch_price"]').val());
    }
