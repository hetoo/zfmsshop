...
CREATE TABLE `#__address` (
  ...
  `area_lat` decimal(17,13) DEFAULT '0.0000000000000' COMMENT '纬度',
  `area_lng` decimal(17,13) DEFAULT '0.0000000000000' COMMENT '经度',
...
(30, '闪购右侧广告位', 0, 2, 1, 358, 200, 0, 0, '30.jpg'),
...
CREATE TABLE `#__chain` (
  ...
  `chain_banner` varchar(50) NULL COMMENT '门店横幅图片',
  `chain_state` tinyint(2) DEFAULT '1' COMMENT '0 已关闭  1 正常  2 待审核 3审核未通过 4待付保证金 5已付保证金 6保证金审核失败',
  `chain_logo` varchar(50) DEFAULT NULL COMMENT '门店LOGO',
  `chain_cycle` int(11) DEFAULT '7' COMMENT '门店结算周期',
  `chain_close_info` varchar(255) DEFAULT NULL COMMENT '门店关闭原因',
  `chain_check_info` varchar(255) DEFAULT NULL COMMENT '审核意见',
  `chain_lat` decimal(17,13) DEFAULT '0.0000000000000' COMMENT '纬度',
  `chain_lng` decimal(17,13) DEFAULT '0.0000000000000' COMMENT '经度',
  `is_transport` tinyint(1) DEFAULT '0' COMMENT '是否支持配送 0不支持  1支持',
  `transport_distance` int(11) DEFAULT '1' COMMENT '配送半径（单位：公里）',
  `start_amount_price` int(11) DEFAULT '0' COMMENT '起送金额',
  `transport_rule` tinyint(1) DEFAULT '1' COMMENT '配送规则  1 配送半径  2配送区域',
  `transport_areas` varchar(255) DEFAULT NULL COMMENT '配送区域 数组',
  `is_forward_order` tinyint(1) DEFAULT '0' COMMENT '是否转接订单 0 否   1是',
  `is_auto_forward` tinyint(1) DEFAULT '0' COMMENT '是否自动转接订单  0否   1是',
  `express_city` text COMMENT '支持发货城市',
  `is_self_take` tinyint(1) DEFAULT '1' COMMENT '是否支持自提 0否  1是',
  `is_collection` tinyint(1) DEFAULT '0' COMMENT '是否支持代收货  0否   1是',
  `collection_price` decimal(10,2) DEFAULT '0.00' COMMENT '代收货费用',
  `store_name` varchar(255) DEFAULT NULL COMMENT '所属店铺名称',
  `chain_apply_time` int(11) DEFAULT '0' COMMENT '门店申请时间',
  `chain_time` int(11) DEFAULT '0' COMMENT '开店时间',
  `chain_close_time` int(11) DEFAULT '0' COMMENT '门店关闭时间',
  `is_own` tinyint(4) DEFAULT '1' COMMENT '是否自营',
  `pay_certificate_type` varchar(20) DEFAULT 'offline' COMMENT '保证金支付方式名称代码 offline线下支付',
  `paying_money_certificate` varchar(255) DEFAULT NULL COMMENT '支付凭证',
  `paying_money_certif_exp` varchar(255) DEFAULT NULL COMMENT '保证金支付说明',
  `express_city_name` text COMMENT '发货城市名称',
  `transport_freight` decimal(12,2) unsigned DEFAULT '0.00' COMMENT '配送费',
  PRIMARY KEY (`chain_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='店铺门店表';

CREATE TABLE `#__chain_cart` (
  `cart_id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '购物车ID',
  `member_id` varchar(20) NOT NULL DEFAULT '0' COMMENT '会员ID,未登录的为u*',
  ...
  `goods_num` smallint(5) unsigned NOT NULL DEFAULT '1' COMMENT '购买商品数量',
  `goods_image` varchar(100) NOT NULL COMMENT '商品图片',
  `goods_selected` tinyint(1) unsigned NOT NULL DEFAULT '1' COMMENT '是否已选:0否,1是',
  PRIMARY KEY (`cart_id`),
  UNIQUE KEY `log_id` (`goods_id`,`chain_id`,`member_id`),
  KEY `member_id` (`member_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='门店购物车数据表';

CREATE TABLE `#__chain_goods_class` (
  `class_id` int(11) NOT NULL AUTO_INCREMENT COMMENT '索引ID',
  `class_name` varchar(50) NOT NULL COMMENT '商品分类名称',
  `class_parent_id` int(11) NOT NULL COMMENT '父级id',
  `class_state` tinyint(1) NOT NULL DEFAULT '0' COMMENT '商品分类状态',
  `chain_id` int(11) NOT NULL DEFAULT '0' COMMENT '门店id',
  `class_sort` int(11) NOT NULL DEFAULT '0' COMMENT '分类排序',
  PRIMARY KEY (`class_id`),
  KEY `class_parent_id` (`class_parent_id`,`class_sort`),
  KEY `chain_id` (`chain_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='门店商品分类表';

CREATE TABLE `#__chain_order_bill` (
  `ob_id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '结算单编号',
  `ob_create_date` int(10) unsigned NOT NULL COMMENT '生成结算单日期',
  `ob_start_date` int(10) unsigned NOT NULL COMMENT '开始日期',
  `ob_end_date` int(10) unsigned NOT NULL COMMENT '结束日期',
  `ob_result_totals` decimal(10,2) NOT NULL DEFAULT '0.00' COMMENT '应结金额',
  `ob_order_totals` decimal(10,2) unsigned NOT NULL DEFAULT '0.00' COMMENT '订单金额',
  `ob_offline_totals` decimal(10,2) unsigned NOT NULL DEFAULT '0.00' COMMENT '线下支付金额',
  `ob_shipping_totals` decimal(10,2) unsigned NOT NULL DEFAULT '0.00' COMMENT '运费',
  `ob_order_return_totals` decimal(10,2) unsigned NOT NULL DEFAULT '0.00' COMMENT '退单金额',
  `ob_commis_totals` decimal(10,2) unsigned NOT NULL DEFAULT '0.00' COMMENT '佣金金额',
  `ob_commis_return_totals` decimal(10,2) unsigned NOT NULL DEFAULT '0.00' COMMENT '退还佣金',
  ...
  `ob_store_id` int(10) unsigned NOT NULL COMMENT '店铺ID',
  `ob_chain_id` int(10) unsigned NOT NULL COMMENT '门店ID',
  `ob_chain_name` varchar(50) DEFAULT NULL COMMENT '门店名称',
  PRIMARY KEY (`ob_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='门店订单结算表';

CREATE TABLE `#__chain_reciver` (
  ...
  `chain_price` decimal(10,2) DEFAULT '0.00' COMMENT '代收费用',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='门店代收订单表';

CREATE TABLE `#__chain_sender_log` (
  `log_id` int(11) NOT NULL AUTO_INCREMENT COMMENT '记录编号',
  `order_id` int(11) NOT NULL COMMENT '订单ID',
  ...
  `chain_sender_state` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '门店发货:0待分派,10待确认,20已确认,30已发货',
  PRIMARY KEY (`log_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='门店发货记录表';
...
CREATE TABLE `#__chain_stock` (
  ...
  `chain_price` decimal(10,2) NOT NULL DEFAULT '0.00' COMMENT '门店价格',
  `cate_id` int(11) unsigned DEFAULT '0' COMMENT '分类ID',
  `p_cate_id` int(11) unsigned DEFAULT '0' COMMENT '顶级分类ID',
...
CREATE TABLE `#__chain_voucher` (
  ...
  `voucher_chain_id` int(11) NOT NULL COMMENT '代金券的门店id',
...
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='门店代金券表';

CREATE TABLE `#__chain_voucher_template` (
  `voucher_t_id` int(11) NOT NULL AUTO_INCREMENT COMMENT '代金券模板编号',
  `voucher_t_title` varchar(50) NOT NULL COMMENT '代金券模板名称',
  `voucher_t_desc` varchar(255) NOT NULL COMMENT '代金券模板描述',
  `voucher_t_start_date` int(11) NOT NULL COMMENT '代金券模板有效期开始时间',
  `voucher_t_end_date` int(11) NOT NULL COMMENT '代金券模板有效期结束时间',
  `voucher_t_price` int(11) NOT NULL COMMENT '代金券模板面额',
  `voucher_t_limit` decimal(10,2) NOT NULL COMMENT '代金券使用时的订单限额',
  `voucher_t_chain_id` int(11) NOT NULL COMMENT '代金券模板的门店id',
  `voucher_t_chain_name` varchar(100) DEFAULT NULL COMMENT '门店名称',
  `voucher_t_state` tinyint(4) NOT NULL COMMENT '代金券模板状态(1-有效,2-失效)',
  `voucher_t_giveout` int(11) NOT NULL COMMENT '模板已发放的代金券数量',
  `voucher_t_used` int(11) NOT NULL COMMENT '模板已经使用过的代金券',
  `voucher_t_add_date` int(11) NOT NULL COMMENT '模板的创建时间',
...
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='门店代金券模板表';
...
('7', 'distribute_auth', '分销商认知协议', '分销商认证协议', '1432522192'),
('8', 'pintuan_doc', '拼团玩法详情', '拼团玩法详情', '1432522192');

CREATE TABLE `#__earnest_money` (
  `etm_id` int(11) NOT NULL AUTO_INCREMENT COMMENT '自增编号',
  `etm_sn` bigint(20) unsigned NOT NULL COMMENT '记录唯一标示',
  `etm_member_id` int(11) NOT NULL COMMENT '会员编号',
  `etm_member_name` varchar(50) NOT NULL COMMENT '会员名称',
  `etm_amount` decimal(10,2) NOT NULL DEFAULT '0.00' COMMENT '缴费金额',
  `etm_payment_code` varchar(20) DEFAULT '' COMMENT '支付方式',
  `etm_payment_name` varchar(15) DEFAULT '' COMMENT '支付方式',
  `etm_trade_sn` varchar(50) DEFAULT '' COMMENT '第三方支付接口交易号',
  `etm_add_time` int(11) NOT NULL COMMENT '添加时间',
  `etm_payment_state` enum('0','1') NOT NULL DEFAULT '0' COMMENT '支付状态 0未支付1支付',
  `etm_payment_time` int(11) NOT NULL DEFAULT '0' COMMENT '支付时间',
  `etm_admin` varchar(30) DEFAULT '' COMMENT '管理员名',
  `etm_chain_id` int(11) DEFAULT '0' COMMENT '门店编号',
  PRIMARY KEY (`etm_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='保证金交付表';

CREATE TABLE `#__earnest_money_log` (
  ...
  `lg_type` varchar(15) NOT NULL DEFAULT '' COMMENT 'store_joinin店铺入驻,open_chain开设门店,close_chain关闭门店,close_store关闭店铺,admin_addition管理员增加，admin_reduction管理员扣减',
  `lg_av_amount` decimal(10,2) NOT NULL DEFAULT '0.00' COMMENT '变更金额 0表示未变更',
  `lg_add_time` int(11) NOT NULL COMMENT '添加时间',
  `lg_desc` varchar(150) DEFAULT NULL COMMENT '描述',
  PRIMARY KEY (`lg_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='保证金变更日志表';

-- Copyright: 天津市网城天创科技有限责任公司	
CREATE TABLE `#__dis_cart` (
  `goods_commonid` int(10) unsigned NOT NULL COMMENT '商品公共表ID',
  `buyer_id` int(10) unsigned NOT NULL COMMENT '买家ID',
  `dis_member_id` int(10) unsigned NOT NULL COMMENT '分销会员ID',
  `end_time` int(10) unsigned NOT NULL COMMENT '到期时间',
  PRIMARY KEY (`goods_commonid`,`buyer_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='分销点击记录表';

CREATE TABLE `#__dis_goods` (
  `distri_id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '分销编号',
...
CREATE TABLE `#__dis_goods_sta` (
  ...
  `dis_commis_amount` decimal(10,2) unsigned NOT NULL DEFAULT '0.00' COMMENT '已结分销佣金',
...
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='分销商品统计表';

CREATE TABLE `#__dis_pay` (
  ...
  `dis_commis_rate` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '分销佣金比例',
  `dis_pay_amount` decimal(10,2) DEFAULT '0.00' COMMENT '分销佣金',
  `dis_pay_time` int(10) unsigned DEFAULT '0' COMMENT '已结时间',
  `log_state` tinyint(1) unsigned DEFAULT '0' COMMENT '结算状态:0为未结,1为已结',
  PRIMARY KEY (`log_id`),
  KEY `order_goods_id` (`order_goods_id`),
  KEY `add_time` (`add_time`),
  KEY `dis_member_id` (`dis_member_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='分销佣金结算表';

CREATE TABLE `#__dis_trad_cash` (
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='分销佣金提现记录表';

CREATE TABLE `#__dis_trad_log` (
  `lg_id` int(11) NOT NULL AUTO_INCREMENT COMMENT '自增编号',
  `lg_member_id` int(11) NOT NULL COMMENT '会员编号',
  `lg_member_name` varchar(50) NOT NULL COMMENT '会员名称',
  `lg_admin_name` varchar(50) DEFAULT NULL COMMENT '管理员名称',
  ...
  `lg_desc` varchar(150) DEFAULT NULL COMMENT '描述',
  PRIMARY KEY (`lg_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='分销佣金变更日志表';

CREATE TABLE `#__dis_web` (
  ...
  PRIMARY KEY (`web_id`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='分销页面模块表';

INSERT INTO `#__dis_web` (`web_id`, `web_name`, `web_page`, `update_time`, `web_sort`) VALUES (101, '首页联动焦点图片', 'index_pic', 1473241265, 100);
INSERT INTO `#__dis_web` (`web_id`, `web_name`, `web_page`, `update_time`, `web_sort`) VALUES (1, '首页头部焦点图片', 'index', 1473241265, 1);
INSERT INTO `#__dis_web` (`web_id`, `web_name`, `web_page`,`update_time`, `web_sort`) VALUES (2, '首页右侧焦点图片', 'index_right_pic', 1473649471, 2);

CREATE TABLE `#__dis_web_code` (
  ...
  PRIMARY KEY (`code_id`),
  KEY `web_id` (`web_id`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='分销模块内容表';

INSERT INTO `#__dis_web_code` (`code_id`, `web_id`, `code_type`, `var_name`, `code_info`, `show_name`) VALUES (101, 1, 'array', 'screen_list', '', '焦点广告图');
INSERT INTO `#__dis_web_code` (`code_id`, `web_id`, `code_type`, `var_name`, `code_info`, `show_name`) VALUES (102, 101, 'array', 'focus_list', '', '三联广告图');
INSERT INTO `#__dis_web_code` (`code_id`, `web_id`, `code_type`, `var_name`, `code_info`, `show_name`) VALUES (103, 101, 'array', 'two_focus_list', '', '两联广告图');
INSERT INTO `#__dis_web_code` (`code_id`, `web_id`, `code_type`, `var_name`, `code_info`, `show_name`) VALUES (104, 101, 'array', 'eight_focus_list', '', '八联广告图');
INSERT INTO `#__dis_web_code` (`code_id`, `web_id`, `var_name`, `show_name`) VALUES (105, 2, 'adv_list', '右侧广告图');
...
CREATE TABLE `#__goods` (
  ...
  `is_batch`  tinyint(1) NULL DEFAULT 0 COMMENT '是否批发商品 0零售  1批发',
  `batch_price`  text COMMENT '批发阶梯价',
  `goods_inv` tinyint(1) unsigned NOT NULL DEFAULT '1' COMMENT '是否开发票',
  `member_price_1` decimal(10,2) unsigned NOT NULL DEFAULT '0.00' COMMENT '会员等级价v1',
  `member_price_2` decimal(10,2) unsigned NOT NULL DEFAULT '0.00' COMMENT '会员等级价v2',
  `member_price_3` decimal(10,2) unsigned NOT NULL DEFAULT '0.00' COMMENT '会员等级价v3',
...
CREATE TABLE `#__goods_common` (
  ...
  `goods_trans_v` decimal(10,2) unsigned NOT NULL DEFAULT '0.00' COMMENT '重量或体积',
  `is_dis` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '是否分销',
  `dis_add_time` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '分销添加时间',
  `dis_commis_rate` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '分销佣金比例',
...
CREATE TABLE `#__lib_goods` (
  ...
  PRIMARY KEY (`goods_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='商品库表';
...
CREATE TABLE `#__lottery_dial_activity` (
  `lot_id` int(11) NOT NULL AUTO_INCREMENT COMMENT '活动序号',
  `lot_name` varchar(255) DEFAULT NULL COMMENT '活动名称',
  `lot_discription` text COMMENT '活动描述',
  `add_time` int(11) DEFAULT NULL COMMENT '活动添加时间',
  `start_time` int(11) DEFAULT NULL COMMENT '活动开始时间',
  `end_time` int(11) DEFAULT NULL COMMENT '活动结束时间',
  `lot_dial_bg` varchar(255) DEFAULT NULL COMMENT '大转盘背景图片',
  `lot_dial_pointer` varchar(255) DEFAULT NULL COMMENT '大转盘指针图片',
  `lot_type` tinyint(2) DEFAULT '0' COMMENT '抽奖用户范围 0 所有会员 1 完成订单',
  `lot_weight` double(8,2) unsigned DEFAULT NULL COMMENT '中奖率',
  `lot_count` int(11) DEFAULT '1' COMMENT '抽取次数 0 不限 ',
  `lot_info` text COMMENT '奖项信息',
  `lot_state` tinyint(2) DEFAULT '0' COMMENT '活动状态 0未开启 1正常 2完成  3关闭',
  `lot_bg` varchar(255) NULL COMMENT '活动背景图片',
  PRIMARY KEY (`lot_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='大转盘活动表';

CREATE TABLE `#__lottery_dial_detail` (
  `act_id` int(11) NOT NULL AUTO_INCREMENT COMMENT '抽奖编号',
  `lot_id` int(11) DEFAULT NULL COMMENT '活动编号',
  ...
  `rate_name` varchar(255) DEFAULT NULL COMMENT '奖项名称',
  `rate_type` tinyint(1) DEFAULT '0' COMMENT '中奖类型 0未中奖 1积分 2卡券 3实物',
  `prize_info` text COMMENT '奖品信息',
  `prize_state` tinyint(1) DEFAULT NULL COMMENT '派奖状态 0 待派奖 1已派奖',
  `prize_time` int(11) DEFAULT NULL COMMENT '发奖时间',
  `prize_member` varchar(50) DEFAULT NULL COMMENT '派奖人 sys系统自动发奖',
  `prize_content` varchar(255) DEFAULT NULL COMMENT '派奖备注',
  PRIMARY KEY (`act_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='抽奖详情表';

CREATE TABLE `#__lottery_dial_detail_1` (
  `act_id` int(11) NOT NULL AUTO_INCREMENT COMMENT '抽奖编号',
  `lot_id` int(11) DEFAULT NULL COMMENT '活动编号',
  `member_id` int(11) DEFAULT NULL COMMENT '会员编号',
  `member_name` varchar(50) DEFAULT NULL COMMENT '会员名称',
...
CREATE TABLE `#__lottery_dial_detail_2` (
  `act_id` int(11) NOT NULL AUTO_INCREMENT COMMENT '抽奖编号',
  `lot_id` int(11) DEFAULT NULL COMMENT '活动编号',
  `member_id` int(11) DEFAULT NULL COMMENT '会员编号',
  `member_name` varchar(50) DEFAULT NULL COMMENT '会员名称',
...
CREATE TABLE `#__lottery_dial_detail_3` (
  `act_id` int(11) NOT NULL AUTO_INCREMENT COMMENT '抽奖编号',
  `lot_id` int(11) DEFAULT NULL COMMENT '活动编号',
  `member_id` int(11) DEFAULT NULL COMMENT '会员编号',
  `member_name` varchar(50) DEFAULT NULL COMMENT '会员名称',
...
CREATE TABLE `#__lottery_dial_detail_4` (
  `act_id` int(11) NOT NULL AUTO_INCREMENT COMMENT '抽奖编号',
  `lot_id` int(11) DEFAULT NULL COMMENT '活动编号',
  `member_id` int(11) DEFAULT NULL COMMENT '会员编号',
  `member_name` varchar(50) DEFAULT NULL COMMENT '会员名称',
...
CREATE TABLE `#__lottery_dial_qualifications` (
  `qua_id` int(11) unsigned NOT NULL AUTO_INCREMENT COMMENT '资格编号',
  `lot_id` int(11) unsigned NOT NULL COMMENT '活动编号',
  `member_id` int(11) unsigned NOT NULL COMMENT '会员编号',
  `lot_type` varchar(15) NOT NULL COMMENT '抽奖活动类型',
  `lot_amount` int(11) unsigned NOT NULL DEFAULT '0' COMMENT '可抽取次数',
  PRIMARY KEY (`qua_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='抽奖资格表';

CREATE TABLE `#__lottery_dial_qualifications_1` (
  `qua_id` int(11) unsigned NOT NULL AUTO_INCREMENT COMMENT '资格编号',
  `lot_id` int(11) unsigned NOT NULL COMMENT '活动编号',
  `member_id` int(11) unsigned NOT NULL COMMENT '会员编号',
  `lot_type` varchar(15) NOT NULL COMMENT '抽奖活动类型',
...
CREATE TABLE `#__lottery_dial_qualifications_2` (
  `qua_id` int(11) unsigned NOT NULL AUTO_INCREMENT COMMENT '资格编号',
  `lot_id` int(11) unsigned NOT NULL COMMENT '活动编号',
  `member_id` int(11) unsigned NOT NULL COMMENT '会员编号',
  `lot_type` varchar(15) NOT NULL COMMENT '抽奖活动类型',
...
CREATE TABLE `#__lottery_dial_qualifications_3` (
  `qua_id` int(11) unsigned NOT NULL AUTO_INCREMENT COMMENT '资格编号',
  `lot_id` int(11) unsigned NOT NULL COMMENT '活动编号',
  `member_id` int(11) unsigned NOT NULL COMMENT '会员编号',
  `lot_type` varchar(15) NOT NULL COMMENT '抽奖活动类型',
...
CREATE TABLE `#__lottery_dial_qualifications_4` (
  `qua_id` int(11) unsigned NOT NULL AUTO_INCREMENT COMMENT '资格编号',
  `lot_id` int(11) unsigned NOT NULL COMMENT '活动编号',
  `member_id` int(11) unsigned NOT NULL COMMENT '会员编号',
  `lot_type` varchar(15) NOT NULL COMMENT '抽奖活动类型',
...
CREATE TABLE `#__member` (
  ...
  `distri_state` tinyint(1) DEFAULT '0' COMMENT '分销状态 0未申请 1待审核 2已通过 3未通过 4清退 5退出',
  ...
  `distri_code` varchar(255) DEFAULT NULL COMMENT '分销代码',
  `distri_time` int(11) DEFAULT NULL COMMENT '申请时间',
  `distri_handle_time` int(11) DEFAULT NULL COMMENT '处理时间',
  `distri_show` tinyint(1) NOT NULL DEFAULT '0' COMMENT '分销中心是否显示 0不显示 1显示',
  `quit_time` int(11) DEFAULT NULL COMMENT '退出时间',
  `distri_apply_times` int(11) DEFAULT '0' COMMENT '申请次数',
  `distri_quit_times` int(11) DEFAULT '0' COMMENT '退出次数',
  `weixin_mp_openid` VARCHAR(50) DEFAULT NULL COMMENT '微信公众号OpenID',
  `is_cash` tinyint(1) DEFAULT '1' COMMENT '是否允许提现，0否，1是',
  `id_card_name` varchar(20) DEFAULT '' COMMENT '实名认证姓名',
  `id_card_code` varchar(20) DEFAULT '' COMMENT '身份证号',
  `id_card_bind` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否实名认证0否,1是',
  `id_card_state` tinyint(1) NOT NULL DEFAULT '0' COMMENT '审核状态0未申请，1待审核，2审核成功，3审核失败',
  `id_card_explain` varchar(50) DEFAULT '' COMMENT '审核说明',
  `id_card_img` varchar(50) DEFAULT '' COMMENT '身份证正反面图片',
  `earnest_money` decimal(12,2) DEFAULT '0.00' COMMENT '保证金金额',
...
CREATE TABLE `#__member_msg_tpl` (
  ...
  `mmt_wx_switch` tinyint(1) unsigned DEFAULT '0' COMMENT '微信接收开关',
...
('trad_change', '佣金变动提醒', '1', '你的账户于 {$time} 账户资金有变化，描述：{$desc}，可用金额变化 ：{$av_amount}元，冻结金额变化：{$freeze_amount}元。<a href=\"{$pd_url}\" target=\"_blank\">点击查看余额</a>', '0', '【{$site_name}】你的账户于 {$time} 账户资金有变化，描述：{$desc}，可用金额变化： {$av_amount}元，冻结金额变化：{$freeze_amount}元。', '0', '{$site_name}提醒：你的账户于 {$time} 账户佣金有变化，描述：{$desc}，可提现佣金金额变化 ：{$av_amount}元，冻结佣金金额变化：{$freeze_amount}元。', '<p>\r\n	{$site_name}提醒：\r\n</p>\r\n<p>\r\n	你的账户于 {$time} 账户佣金有变化，描述：{$desc}，可提现佣金金额变化 ：{$av_amount}元，冻结佣金金额变化：{$freeze_amount}元。\r\n</p>\r\n<p>\r\n	<a href=\"{$trad_url}\" target=\"_blank\">点击查看详情</a> \r\n</p>\r\n<p>\r\n	<br />\r\n</p>\r\n<p>\r\n	<br />\r\n</p>\r\n<p>\r\n	<br />\r\n</p>\r\n<p style=\"text-align:right;\">\r\n	{$site_name}\r\n</p>\r\n<p style=\"text-align:right;\">\r\n	{$mail_send_time}\r\n</p>');
...
(10, 0, '分销市场', 'distribute', 1, 1, 255, 0),
...
CREATE TABLE `#__orders` (
  ...
  `order_type` tinyint(4) DEFAULT '1' COMMENT '订单类型1普通订单(默认),2预定订单,3门店自提,4拼团订单,5门店配送',
  ...
  `is_dis` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '是否分销订单',
  `chain_sender_state` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '门店发货:0待分派,10待确认,20已确认,30已发货',
  PRIMARY KEY (`order_id`)
...
CREATE TABLE `#__order_bill` (
  ...
  `ob_rpt_amount` decimal(10,2) unsigned NOT NULL DEFAULT '0.00' COMMENT '下单时使用的红包值',
  `ob_rf_rpt_amount` decimal(10,2) unsigned NOT NULL DEFAULT '0.00' COMMENT '全部退款时红包值',
  `ob_dis_pay_amount` decimal(10,2) unsigned DEFAULT '0.00' COMMENT '分销佣金',
  `ob_offline_totals` decimal(10,2) unsigned NOT NULL DEFAULT '0.00' COMMENT '线下支付金额',
  PRIMARY KEY (`ob_id`)
...
CREATE TABLE `#__order_common` (
  ...
  `reciver_date_msg` varchar(20) DEFAULT '' COMMENT '希望送货时间',
  PRIMARY KEY (`order_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='订单信息扩展表';

CREATE TABLE `#__order_e_waybill` (
  `order_id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '订单id',
  `kdn_order_code` varchar(50) NOT NULL COMMENT '快递鸟订单号',
  `logistic_code` varchar(50) NOT NULL COMMENT '快递单号',
  `print_template` text COMMENT '面单打印模板',
  `insert_time` int(10) unsigned NOT NULL COMMENT '添加时间',
  PRIMARY KEY (`order_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='电子面单表';
...
CREATE TABLE `#__order_goods` (
  ...
  `goods_type` tinyint(1) unsigned NOT NULL DEFAULT '1' COMMENT '1默认2团购商品3限时折扣商品4组合套装5赠品8加价购活动商品9加价购换购商品10拼团',
  ...
  `is_dis` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '是否分销商品',
  `dis_commis_rate` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '分销佣金比例',
  `dis_member_id` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '分销会员ID',
  PRIMARY KEY (`rec_id`),
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='拼团订单参团表';
...
CREATE TABLE `#__p_pintuan` (
  `pintuan_id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '活动编号',
  `pintuan_name` varchar(50) NOT NULL COMMENT '活动名称',
  `pintuan_title` varchar(20) DEFAULT NULL COMMENT '活动标题',
  `pintuan_explain` varchar(250) DEFAULT NULL COMMENT '活动说明',
...
CREATE TABLE `#__p_pintuan_goods` (
  `pintuan_goods_id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '拼团商品编号',
  `pintuan_id` int(10) unsigned NOT NULL COMMENT '活动编号',
  `pintuan_name` varchar(50) NOT NULL COMMENT '活动名称',
  `pintuan_title` varchar(10) DEFAULT NULL COMMENT '活动标题',
  `pintuan_explain` varchar(50) DEFAULT NULL COMMENT '活动说明',
  ...
  PRIMARY KEY (`pintuan_goods_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='拼团商品表';

CREATE TABLE `#__p_pintuan_quota` (
...
CREATE TABLE `#__p_flash` (
  `flash_id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '闪购编号',
  `flash_name` varchar(50) NOT NULL COMMENT '活动名称',
  `flash_title` varchar(10) DEFAULT NULL COMMENT '活动标题',
  `flash_explain` varchar(50) DEFAULT NULL COMMENT '活动说明',
  `quota_id` int(10) unsigned NOT NULL COMMENT '套餐编号',
  `start_time` int(10) unsigned NOT NULL COMMENT '活动开始时间',
  `end_time` int(10) unsigned NOT NULL COMMENT '活动结束时间',
  `member_id` int(10) unsigned NOT NULL COMMENT '用户编号',
  ...
  `member_name` varchar(50) NOT NULL COMMENT '用户名',
  `store_name` varchar(50) NOT NULL COMMENT '店铺名称',
  `upper_limit` int(10) unsigned NOT NULL DEFAULT '1' COMMENT '购买上限，0为不限制',
  `flash_state` tinyint(3) unsigned NOT NULL DEFAULT '1' COMMENT '状态，0-取消 1-正常',
  `flash_brand` varchar(150) DEFAULT NULL COMMENT '闪购品牌',
  `flash_pic` varchar(150) DEFAULT NULL COMMENT '闪购图片',
  `flash_banner` varchar(150) DEFAULT NULL COMMENT '闪购横幅',
  `flash_recommend_pic` varchar(150) DEFAULT NULL COMMENT '闪购推荐',
  `is_recommend` tinyint(1) unsigned DEFAULT '0' COMMENT '是否推荐',
  PRIMARY KEY (`flash_id`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='闪购活动表';

CREATE TABLE `#__p_flash_goods` (
  `flash_goods_id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '闪购商品表索引',
  `flash_id` int(10) unsigned NOT NULL COMMENT '闪购活动编号',
  `flash_name` varchar(50) NOT NULL COMMENT '活动名称',
  `flash_title` varchar(10) DEFAULT NULL COMMENT '活动标题',
  `flash_explain` varchar(50) DEFAULT NULL COMMENT '活动说明',
  `goods_id` int(10) unsigned NOT NULL COMMENT '商品编号',
  ...
  `goods_name` varchar(100) NOT NULL COMMENT '商品名称',
  `goods_price` decimal(10,2) NOT NULL COMMENT '店铺价格',
  `flash_price` decimal(10,2) NOT NULL COMMENT '闪购价格',
  `goods_image` varchar(100) NOT NULL COMMENT '商品图片',
  `start_time` int(10) unsigned NOT NULL COMMENT '开始时间',
  `end_time` int(10) unsigned NOT NULL COMMENT '结束时间',
  `upper_limit` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '购买上限，0为不限制',
  `flash_state` tinyint(3) unsigned NOT NULL DEFAULT '1' COMMENT '状态，0-取消 1-正常',
  `flash_recommend` tinyint(3) unsigned NOT NULL DEFAULT '0' COMMENT '推荐标志 0-未推荐 1-已推荐',
  `gc_id_1` mediumint(9) DEFAULT '0' COMMENT '商品分类一级ID',
  `flash_amount` int(10) unsigned DEFAULT '1' COMMENT '闪购总数',
  `buy_count` int(10) unsigned DEFAULT '0' COMMENT '已购买数量',
  PRIMARY KEY (`flash_goods_id`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='闪购商品表';

CREATE TABLE `#__p_flash_quota` (
  `quota_id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '闪购套餐编号',
  `member_id` int(10) unsigned NOT NULL COMMENT '用户编号',
  `store_id` int(10) unsigned NOT NULL COMMENT '店铺编号',
  `member_name` varchar(50) NOT NULL COMMENT '用户名',
  `store_name` varchar(50) NOT NULL COMMENT '店铺名称',
  ...
  `end_time` int(10) unsigned NOT NULL COMMENT '套餐结束时间',
  PRIMARY KEY (`quota_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='闪购套餐表';
...
CREATE TABLE `#__p_spike` (
  `spike_id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '秒杀编号',
  `spike_name` varchar(50) NOT NULL COMMENT '活动名称',
  `spike_title` varchar(10) DEFAULT NULL COMMENT '活动标题',
  `spike_explain` varchar(50) DEFAULT NULL COMMENT '活动说明',
  `quota_id` int(10) unsigned NOT NULL COMMENT '套餐编号',
  `start_time` int(10) unsigned NOT NULL COMMENT '活动开始时间',
  `end_time` int(10) unsigned NOT NULL COMMENT '活动结束时间',
  `member_id` int(10) unsigned NOT NULL COMMENT '用户编号',
  ...
  `store_name` varchar(50) NOT NULL COMMENT '店铺名称',
  `upper_limit` int(10) unsigned NOT NULL DEFAULT '1' COMMENT '购买上限，0为不限制',
  `spike_state` tinyint(3) unsigned NOT NULL DEFAULT '1' COMMENT '状态，0-取消 1-正常',
  `spike_banner` varchar(255) DEFAULT NULL COMMENT '品牌秒杀横幅',
  `spike_common_bg` varchar(255) DEFAULT NULL COMMENT '品牌秒杀推荐背景',
  `order_limit` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '订单限制，0为不限制',
  PRIMARY KEY (`spike_id`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='秒杀活动表';

CREATE TABLE `#__p_spike_goods` (
  `spike_goods_id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '秒杀商品表索引',
  `spike_id` int(10) unsigned NOT NULL COMMENT '秒杀活动编号',
  `spike_name` varchar(50) NOT NULL COMMENT '活动名称',
  `spike_title` varchar(10) DEFAULT NULL COMMENT '活动标题',
  `spike_explain` varchar(50) DEFAULT NULL COMMENT '活动说明',
  `goods_id` int(10) unsigned NOT NULL COMMENT '商品编号',
  ...
  `upper_limit` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '购买上限，0为不限制',
  `spike_state` tinyint(3) unsigned NOT NULL DEFAULT '1' COMMENT '状态，0-取消 1-正常',
  `spike_recommend` tinyint(3) unsigned NOT NULL DEFAULT '0' COMMENT '推荐标志 0-未推荐 1-已推荐',
  `gc_id_1` mediumint(9) DEFAULT '0' COMMENT '商品分类一级ID',
  `spike_amount` int(10) unsigned DEFAULT '1' COMMENT '秒杀总数',
  `had_spiked_count` int(10) unsigned DEFAULT '0' COMMENT '已秒杀数量',
  `order_limit` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '订单限制，0为不限制',
  PRIMARY KEY (`spike_goods_id`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='秒杀商品表';

CREATE TABLE `#__p_spike_quota` (
  `quota_id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '秒杀套餐编号',
  `member_id` int(10) unsigned NOT NULL COMMENT '用户编号',
  `store_id` int(10) unsigned NOT NULL COMMENT '店铺编号',
  `member_name` varchar(50) NOT NULL COMMENT '用户名',
  `store_name` varchar(50) NOT NULL COMMENT '店铺名称',
  ...
  `end_time` int(10) unsigned NOT NULL COMMENT '套餐结束时间',
  PRIMARY KEY (`quota_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='秒杀套餐表';

CREATE TABLE `#__p_time` (
  `log_id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '记录编号',
  `start_time` int(10) unsigned NOT NULL COMMENT '开始时间',
  `end_time` int(10) unsigned NOT NULL COMMENT '结束时间',
  `store_id` int(10) unsigned NOT NULL COMMENT '店铺编号',
  `goods_id` int(10) unsigned NOT NULL COMMENT '商品编号',
  `promotion_id` int(10) unsigned NOT NULL COMMENT '促销编号',
  `promotion_price` decimal(10,2) NOT NULL COMMENT '促销价格',
  `promotion_type` tinyint(3) unsigned NOT NULL COMMENT '促销类型2限时折扣,3秒杀,4闪购',
  `is_update` tinyint(1) unsigned DEFAULT '0' COMMENT '是否已更新',
  PRIMARY KEY (`log_id`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='价格促销时间表';
...
CREATE TABLE `#__realtime_log` (
  `log_id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '记录ID',
  `to_id` varchar(50) NOT NULL COMMENT '邮箱地址,手机号,公众号用户openid',
  `subject` varchar(99) DEFAULT '' COMMENT '邮件标题,微信模板编号',
  `log_msg` text NOT NULL COMMENT '内容',
  `msg_type` tinyint(1) unsigned DEFAULT '1' COMMENT '类型:1邮件,2短信,3微信',
  `add_time` int(10) unsigned DEFAULT '0' COMMENT '添加时间',
  `log_type` tinyint(1) unsigned DEFAULT '1' COMMENT '来源:1商城发出,2微信推送',
  PRIMARY KEY (`log_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='消息记录表';

CREATE TABLE `#__realtime_msg` (
  `log_id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '记录ID',
  `to_id` varchar(50) NOT NULL COMMENT '邮箱地址,手机号,公众号用户openid',
  `subject` varchar(99) DEFAULT '' COMMENT '邮件标题,微信模板编号',
  `log_msg` text NOT NULL COMMENT '内容',
  ...
  `add_time` int(10) unsigned DEFAULT '0' COMMENT '添加时间',
  PRIMARY KEY (`log_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='等待发送消息表';
...
CREATE TABLE `#__refund_return` (
  ...
  `is_dis` tinyint(1) unsigned DEFAULT '0' COMMENT '是否分销商品',
  `dis_commis_rate` tinyint(1) unsigned DEFAULT '0' COMMENT '分销佣金比例',
  `goods_commonid` int(10) unsigned DEFAULT '0' COMMENT '商品公共表ID',
  `dis_pay_state` tinyint(1) unsigned DEFAULT '0' COMMENT '分销结算状态:0为未结,1为已结',
  `finnshed_time` int(10) unsigned DEFAULT '0' COMMENT '完成时间,结算用',
  `chain_id` int(10) unsigned DEFAULT '0' COMMENT '门店ID',
  PRIMARY KEY (`refund_id`)
...
CREATE TABLE `#__sensitive_word` (
  `word_id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '敏感词ID',
  `word_name` varchar(20) NOT NULL COMMENT '敏感词',
  `is_open` tinyint(1) unsigned DEFAULT '1' COMMENT '是否开启，0为否，1为是，默认为1',
  `insert_time` int(10) unsigned NOT NULL COMMENT '添加时间',
  PRIMARY KEY (`word_id`)
...
CREATE TABLE `#__store` (
  ...
  `default_im` int(10) unsigned NOT NULL COMMENT '默认客服帐号',
  `order_price_modify` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '是否允许修改订单金额',
  `order_paid_refund` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '是否允许取消已付款订单',
  `is_chain_allow` tinyint(1) DEFAULT '1' COMMENT '是否允许添加门店',
  `allow_chain_count` int(11) unsigned DEFAULT '5' COMMENT '允许添加门店数量',
  `earnest_money` int(11) unsigned DEFAULT '0' COMMENT '保证金金额[不设置则按照平台统一规则收取]',
  `special_business` tinyint(2) NOT NULL DEFAULT '0' COMMENT '特殊行业资质:0无，1医疗行业，2食品行业，3书籍音像，4酒类制品',
  PRIMARY KEY (`store_id`),
...
CREATE TABLE `#__store_extend` (
  ...
  `dis_commis_rate` tinyint(1) unsigned NOT NULL DEFAULT '5' COMMENT '分销佣金比例',
  PRIMARY KEY (`store_id`)
...
CREATE TABLE `#__store_joinin` (
  ...
  `special_business` tinyint(2) NOT NULL DEFAULT '0' COMMENT '特殊行业资质:0无，1医疗行业，2食品行业，3书籍音像，4酒类制品',
  `business_licence_address_detail` varchar(255) NOT NULL COMMENT '营业执照详细地址',
  `legal_person_name` varchar(50) NOT NULL COMMENT '法人姓名',
  `legal_person_card_id` varchar(20) NOT NULL COMMENT '法人身份证号',
  `card_start_time` date NOT NULL COMMENT '身份证有效期开始时间',
  `card_end_time` date NOT NULL COMMENT '身份证有效期结束时间',
  `card_before_elc` varchar(50) NOT NULL COMMENT '身份证电子版正面',
  `card_back_elc` varchar(50) NOT NULL COMMENT '身份证电子版反面',
  `other_qualification_electronic_1` varchar(50) DEFAULT NULL COMMENT '其他资质电子版（一）',
  `other_qualification_electronic_2` varchar(50) DEFAULT NULL COMMENT '其他资质电子版（二）',
  PRIMARY KEY (`member_id`)
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='媒体库视频表';
...
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='媒体库表';

INSERT INTO `#__video_album_class` (`video_class_name`, `store_id`, `video_class_des`, `video_class_sort`, `upload_time`, `is_default`) VALUES ('商品库默认媒体库', 0, '商品库使用，请勿删除', 0, 1432544056, 1);
...
(5, '蓝色', 'blue', 'index_fl', 1432522192, 4, 0, NULL),
(6, '紫色', 'purple', 'index_fl', 1432522192, 6, 0, NULL),
(7, '褐色', 'brown', 'index_fl', 1432522192, 7, 0, NULL),
(8, '默认', 'default', 'index_fl', 1432522192, 8, 0, NULL),
...
(501, 0, 'array', 'spike_pic', '', '秒杀头部图片'),
(502, 0, 'array', 'spike_list', '', '秒杀商品推荐'),
(503, 0, 'array', 'flash_pic', '', '闪购头部图片'),
...
CREATE TABLE `#__wx_msg_tpl` (
  ...
  UNIQUE KEY `wx_tpl_id` (`wx_tpl_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='微信模板消息';

INSERT INTO `#__wx_msg_tpl` (`msg_code`, `msg_name`, `wx_tpl_id`, `wx_title`, `wx_content`, `wx_example`, `mp_msg_id`) VALUES
