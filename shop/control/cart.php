...
class cartControl extends BaseBuyControl {
    ...
    public function addOp() {
        ...
        if (is_numeric($_GET['goods_id'])) {
            ...
            if($_SESSION['member_id'] && $goods_info['promotion_info']['promotion_type']==3) {
                $_n = Model('order')->getSpikeOrderLimit($_SESSION['member_id'],$goods_info);
                if($_n > 0) {
                    exit(json_encode(array('msg'=>'抱歉，秒杀商品只能购买'.$_n.'次，请重新选择','spike'=>'1')));
                }
            }
    ...
    public function add_combOp() {
        ...
        //限时折扣
        $logic_buy_1->getPromotionCartList($goods_list);
