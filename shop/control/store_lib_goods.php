...
class store_lib_goodsControl extends BaseSellerControl {
    ...
    public function indexOp() {
        ...
        if (!$is_all) {
            $_class = array();
            $bind_class_list = Model('store_bind_class')->getStoreBindClassList(array('store_id'=> $store_id,'state'=>array('in',array(1,2))));
        ...
        $list = $model_goods->getGoodsList($condition,10);
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'lib_goods':
                $menu_array = array(
                    array('menu_key'=>'index','menu_name'=>'商品库的商品 ',  'menu_url'=>'index.php?act=store_lib_goods&op=index')
                );
                break;
        }
