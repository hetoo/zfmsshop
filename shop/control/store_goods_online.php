...
class store_goods_onlineControl extends BaseSellerControl {
    ...
    public function goods_listOp() {
        ...
        $where = array();
        $where['goods_common.store_id'] = $_SESSION['store_id'];
        if (intval($_GET['stc_id']) > 0) {
            $where['goods_common.goods_stcids'] = array('like', '%,' . intval($_GET['stc_id']) . ',%');
        }
        if (trim($_GET['keyword']) != '') {
            switch ($_GET['search_type']) {
                case 0:
                    $where['goods_common.goods_name'] = array('like', '%' . trim($_GET['keyword']) . '%');
                    break;
                case 1:
                    $where['goods_common.goods_serial'] = array('like', '%' . trim($_GET['keyword']) . '%');
                    break;
                case 2:
                    $where['goods_common.goods_commonid'] = intval($_GET['keyword']);
                    break;
            }
        }
        if (intval($_GET['sup_id']) > 0) {
            $where['goods_common.sup_id']= intval($_GET['sup_id']);
        }
        ...
        if (!$_SESSION['seller_gc_limits'] && $_SESSION['seller_group_id']) {
            ...
            $where['goods_common.gc_id'] = array('in',array_keys($gc_list));
        }
        //库存区间查询
        if (intval($_GET['storage_b']) >= 0 && intval($_GET['storage_e']) >= 1) {
            $where['storage'] = array(intval($_GET['storage_b']),intval($_GET['storage_e']));
        }
    ...
    public function edit_goodsOp() {
        ...
        $goodscommon_info['batch_price'] = unserialize($goodscommon_info['batch_price']);
        ...
        $menu_promotion = array(
            ...
            'member_price' => $goodscommon_info['is_virtual'] == 1 || $goodscommon_info['is_batch'] == 1 ? false : true
        );
    ...
    public function edit_imageOp() {
        ...
        $menu_promotion = array(
                ...
                'member_price' => $common_list['is_virtual'] == 1 || $common_list['is_batch'] == 1 ? false : true
        );
     ...
     * 设置会员等级价
     */
    public function edit_member_priceOp() {
        $common_id = intval($_GET['commonid']);
        if ($common_id <= 0) {
            showMessage(L('wrong_argument'), urlShop('seller_center'), 'html', 'error');
        }
        ...
        $this->profile_menu('edit_detail', 'edit_member_price', $menu_promotion);
        Tpl::showpage('store_goods_edit.edit_member_price');
    }

    /**
     * 保存会员等级价格
     */
    public function save_member_priceOp(){
        if (!chksubmit()) {
            showDialog(L('wrong_argument'));
        }
        $common_id = intval($_POST['commonid']);
        ...
        $model_goods = Model('goods');
        $goodscommon_info = $model_goods->getGoodsCommonInfoByID($common_id);
        if(empty($goodscommon_info) || $goodscommon_info['is_virtual'] == 1 || $goodscommon_info['is_batch'] == 1){
            showDialog('该商品不能设置会员等级价格');
        }

        $result = Logic('goods')->saveMemberPrice($_POST);
    ...
    private function profile_menu($menu_type,$menu_key, $allow_promotion = array()) {
        ...
        switch ($menu_type) {
            ...
            case 'edit_detail':
                ...
                if ($allow_promotion['member_price'] == true) {
                    $menu_array[] = array('menu_key' => 'edit_member_price', 'menu_name' => '会员等级价格', 'menu_url' => urlShop('store_goods_online', 'edit_member_price', array('commonid' => $_GET['commonid'], 'ref_url' => ($_GET['ref_url'] ? $_GET['ref_url'] : getReferer()))) );
                }
                break;
            case 'edit_class':
                $menu_array = array(
