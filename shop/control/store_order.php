...
class store_orderControl extends BaseSellerControl {
    ...
    public function indexOp() {
        ...
        $model_order->setStoreInfo($this->store_info);//设置店铺功能开关
    ...
    public function show_orderOp() {
        ...
        $order_info['if_seller_refund_cancel'] = $model_order->getOrderOperateState('seller_refund_cancel',$order_info);//订单退款
        ...
        if ($order_info['if_store_send'] && $order_info['pintuan_info']['lock_state'] == 1) {////拼团订单
    ...
    public function change_stateOp() {
        ...
        if ($_GET['state_type'] == 'order_cancel') {
            ...
            $result = $this->_order_price($order_info,$_POST);
        }
        if (!$result['state']) {
    ...
    private function _order_cancel($order_info, $post) {
        ...
        $order_list[$order_id] = $order_info;
        $order_list = Model('refund_return')->getGoodsRefundList($order_list,1);//订单商品的退款
        $order_info = $order_list[$order_id];
        $order_info['if_seller_refund_cancel'] = $model_order->getOrderOperateState('seller_refund_cancel',$order_info);//订单退款
        if(!chksubmit()) {
            Tpl::output('order_info',$order_info);
            ...
            exit();
             ...
             if ($order_info['if_seller_refund_cancel'] == 1) {//订单退款
                 return $logic_order->sellerOrderRefund($order_info,$post);
             }
     ...
     * @param unknown $order_info
     */
    private function _order_price($order_info, $post) {
        $model_order = Model('order');
        $logic_order = Logic('order');
        if(!chksubmit()) {
            Tpl::output('order_info',$order_info);
            ...
            return $logic_order->changeOrderPrice($order_info,'seller',$_SESSION['seller_name'],$post);
        }
    ...
    public function order_printOp() {
        ...
        //优惠金额
        $promotion_amount = ncPriceFormat($goods_total_price+$order_info['shipping_fee']) - $order_info['order_amount'];
        //运费
     ...
     * 批量打印发货单
     */
    public function batch_order_printOp() {
        Language::read('member_printorder');
        ...
        foreach ($order_list as $order_id=>$order_info){
            ...
            $data['goods_all_num'] = $goods_all_num;
            $data['goods_total_price'] = ncPriceFormat($goods_total_price);
            $data['goods_list'] = $goods_new_list;
            $order_list_new[$order_id] = $data;
        }
        Tpl::output('order_list',$order_list_new);
        ...
        Tpl::output('store_info',$store_info);
        Tpl::showpage('store_order.batch_print',"null_layout");
    }
