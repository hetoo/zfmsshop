...
class paymentControl extends BaseHomeControl{
     ...
     * 店铺入驻支付保证金
     */
    public function join_payOp(){
        $etm_sn = $_POST['etm_sn'];
        $payment_code = $_POST['payment_code'];
        $url = urlShop('store_joinin');

        if(!preg_match('/^\d{18}$/',$etm_sn)){
            showMessage('参数错误',$url,'html','error');
        }
        ...
        $payment_info = $result['data'];
        $result = $logic_payment->getSEarnetMoneyOrderInfo($etm_sn,$_SESSION['member_id']);
        if(!$result['state']) {
            showMessage($result['msg'], $url, 'html', 'error');
        }
        if ($result['data']['etm_payment_state'] || empty($result['data']['api_pay_amount'])) {
            showMessage('该店铺不需要支付保证金', $url, 'html', 'error');
        }
    ...
    public function chain_payOp(){
        ...
        $payment_info = $result['data'];
        $result = $logic_payment->getCEarnetMoneyOrderInfo($etm_sn,$_SESSION['member_id']);
        if(!$result['state']) {
            showMessage($result['msg'], $url, 'html', 'error');
        }
        if ($result['data']['etm_payment_state'] || empty($result['data']['api_pay_amount'])) {
            showMessage('该门店不需要支付保证金', $url, 'html', 'error');
        }
    ...
    private function _api_pay($order_info, $payment_info) {
        ...
        if($payment_info['payment_code'] == 'chinabank') {
            ...
            } elseif ($order_info['order_type'] == 'pd_order') { //预存款存值
                Tpl::output('pdr_info',$order_info);
                Tpl::output('order_type','pd_order');
                Tpl::output('args','buyer_id='.$_SESSION['member_id'].'&pdr_id='.$order_info['pdr_id']);

            } elseif ($order_info['order_type'] == 'etms_order') { //店铺入住保证金
                Tpl::output('etms_info',$order_info);
                Tpl::output('order_type','etms_order');
                Tpl::output('args','buyer_id='.$_SESSION['member_id'].'&etms_id='.$order_info['etm_id']);

            } elseif ($order_info['order_type'] == 'etmc_order') { //门店保证金
                Tpl::output('etmc_info',$order_info);
                Tpl::output('order_type','etmc_order');
                Tpl::output('args','buyer_id='.$_SESSION['member_id'].'&etmc_id='.$order_info['etm_id']);
    ...
    public function notifyOp(){
        ...
        } elseif ($order_type == 'etms_order') {

            $result = $logic_payment->getSEarnetMoneyOrderInfo($out_trade_no);
            if ($result['data']['etm_payment_state'] == 1) {
                exit($success);
            }
            $api_pay_amount = $result['data']['etm_amount'];

        } elseif ($order_type == 'etmc_order') {

            $result = $logic_payment->getCEarnetMoneyOrderInfo($out_trade_no);
            if ($result['data']['etm_payment_state'] == 1) {
                exit($success);
            }
            $api_pay_amount = $result['data']['etm_amount'];
        ...
        if ($order_type == 'real_order') {
            ...
            $result = $logic_payment->updateSEarnetMoneyOrder($out_trade_no,$trade_no,$payment_info,$order_pay_info);
        } elseif ($order_type == 'etmc_order') {
            $result = $logic_payment->updateCEarnetMoneyOrder($out_trade_no,$trade_no,$payment_info,$order_pay_info);
        }
        if ($result['state']) {
            //记录消费日志
            if ($order_type == 'real_order') {
            ...
            if(!in_array($order_type,array('etmc_order','etms_order')))
    ...
    public function returnOp(){
        ...
        if ($order_type == 'real_order') {
        ...
        } elseif ($order_type == 'etms_order') { //店铺入驻
            ...
            $result['data']['api_pay_amount'] = $result['data']['etm_amount'];

        } elseif ($order_type == 'etmc_order') { //开通门店
        ...
        if ($order_type == 'real_order') {
            ...
            $pay_ok_url = urlShop('store_joinin');
        } elseif ($order_type == 'etmc_order') {
            $pay_ok_url = urlShop('store_chain');
        }
    ...
    public function query_stateOp() {
        ...
        } elseif (intval($_GET['etmc_id']) > 0) {
            $info = Model('earnest_money')->getEarnestMoneyInfo(array('etm_id'=>intval($_GET['etmc_id']),'etm_member_id'=>intval($_GET['buyer_id'])));
            exit(json_encode(array('state'=>($info['etm_payment_state'] == '1'),'pay_sn'=>$info['etm_sn'],'type'=>'c')));
        } elseif (intval($_GET['etms_id']) > 0) {
            $info = Model('earnest_money')->getEarnestMoneyInfo(array('etm_id'=>intval($_GET['etms_id']),'etm_member_id'=>intval($_GET['buyer_id'])));
            exit(json_encode(array('state'=>($info['etm_payment_state'] == '1'),'pay_sn'=>$info['etm_sn'],'type'=>'s')));
        }
    }
}
