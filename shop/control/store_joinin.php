...
class store_joininControl extends BaseHomeControl {
    ...
    public function step2Op() {
        if(!empty($_POST)) {
            ...
            $param['business_licence_address_detail'] = $_POST['business_licence_address_detail'];
            ...
            $param['special_business'] = intval($_POST['special_business']);

            $param['other_qualification_electronic_1'] = $_POST['other_qualification_electronic_11'];
			$param['other_qualification_electronic_2'] = $_POST['other_qualification_electronic_21'];

			$param['legal_person_name'] = $_POST['legal_person_name'];
			$param['legal_person_card_id'] = $_POST['legal_person_card_id'];
			$param['card_start_time'] = $_POST['card_start_time'];
			$param['card_end_time'] = $_POST['card_end_time'];
			$param['card_before_elc'] = $_POST['card_before_elc1'];
			$param['card_back_elc'] = $_POST['card_back_elc1'];
    ...
    private function step2_save_valid($param) {
        ...
        $obj_validate->validateparam = array(
            ...
            array("input"=>$param['business_licence_number'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"20","message"=>"注册号不能为空且必须小于20个字"),
            array("input"=>$param['business_licence_address'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"50","message"=>"营业执照所在地不能为空且必须小于50个字"),
            array("input"=>$param['business_licence_address_detail'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"50","message"=>"营业执照详细地址不能为空且必须小于50个字"),
            ...
            array("input"=>$param['legal_person_name'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"50","message"=>"法人姓名不能为空且必须小于50个字"),
            array("input"=>$param['legal_person_card_id'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"50","message"=>"身份证号不能为空且身份证号必须为18位"),
            array("input"=>$param['card_start_time'], "require"=>"true","message"=>"身份证有效期不能为空"),
            array("input"=>$param['card_end_time'], "require"=>"true","message"=>"身份证有效期不能为空"),
            array("input"=>$param['card_before_elc'], "require"=>"true","message"=>"身份证正面电子版文件不能为空"),
            array("input"=>$param['card_back_elc'], "require"=>"true","message"=>"身份证反面电子版文件不能为空"),
    ...
    public function check_seller_name_existOp() {
        ...
        foreach ($sensitive_list as $v) {
            if($v['name'] == 'seller_name'){
                $is_open = $v['is_open'];
                break;
            }
        }
        //开关
        if($is_open == 1 && sensitiveWordDetection($_GET['seller_name'])){
            echo 'true';exit;
        }
    ...
    public function payOp() {
        ...
        //特殊行业资质
        switch ($this->joinin_detail['special_business']){
            case '1' :
                $this->joinin_detail['special_business_str'] = '医疗行业';
                break;
            case '2' :
                $this->joinin_detail['special_business_str'] = '食品行业';
                break;
            case '3' :
                $this->joinin_detail['special_business_str'] = '书籍音像';
                break;
            case '4' :
                $this->joinin_detail['special_business_str'] = '酒类制品';
                break;
            default :
                $this->joinin_detail['special_business_str'] = '无';
        }
    ...
    if($pay_code == 'offline'){
        $update_param = array();
        $update_param['joinin_state'] = STORE_JOIN_STATE_PAY;
        $model_store_joinin = Model('store_joinin');
        $update_param['paying_money_certificate'] = $this->upload_image('paying_money_certificate');
        $update_param['paying_money_certif_exp'] = $_POST['paying_money_certif_exp'];

        if(empty($update_param['paying_money_certificate'])) {
            showMessage('请上传付款凭证','','','error');
        }
        $model_store_joinin->modify($update_param, array('member_id'=>$_SESSION['member_id']));
    }else{
        $param = array();
        $member_id = $_SESSION['member_id'];
        $param['etm_member_id'] = $member_id;
        $param['etm_member_name'] = $_SESSION['member_name'];
        $param['etm_amount'] = $this->joinin_detail['paying_amount'];
        $param['etm_add_time'] = TIMESTAMP;
        $param['etm_sn'] = $this->_makePaySn($member_id);

        $res = Model('earnest_money')->addEarnestMoney($param);
        if($res){
            $html = '<html><head></head><body>';
            $html .= '<form method="post" name="E_FORM" action="'.urlShop('payment','join_pay').'">';
            ...
            showMessage('支付失败','index.php?act=store_joinin');
        }
    }
    @header('location: index.php?act=store_joinin');
}
