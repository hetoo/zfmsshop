...
class store_billControl extends BaseSellerControl {
    ...
    public function show_billOp(){
        ...
        if ($_GET['type'] =='refund'){
            ...
            $list = $model_dis_order->getDisPayList($condition,20);
            Tpl::output('list',$list);
            Tpl::output('show_page',$model_dis_order->showpage());
            $sub_tpl_name = 'store_bill.show.dis_list';
            $this->profile_menu('show','dis_list');
        ...
        switch ($menu_type) {
            ...
            case 'show':
                ...
                if (C('distribute_isuse') == 1) {
                    array_push($menu_array,array('menu_key'=>'dis_list','menu_name'=>'分销佣金', 'menu_url'=>'index.php?act=store_bill&op=show_bill&type=dis&ob_id='.$_GET['ob_id']));
                }
                break;
        }
