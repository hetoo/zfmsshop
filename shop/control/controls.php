...
if(preg_match("/mobile|nokia|iphone|ipad|android|samsung|htc|blackberry/i", $_SERVER['HTTP_USER_AGENT']) && empty($_GET['pc']) && !preg_match('/item-(\d+).html/',$_SERVER['REQUEST_URI']) && !in_array($_REQUEST['act'], array('mb_app','distri_goods','goods')) && empty($_COOKIE['pc'])) {
    @header('Location: '.C('wap_site_url'));exit;
}
if ($_GET['pc']) setcookie('pc',1,TIMESTAMP+3600,'/');
class Control{
    /**
     * 检查短消息数量
     *
    ...
    private function _getMenuList() {
        $menu_list = array(
            ...
            'promotion' => array('name' => '促销', 'child' => array(
                ...
                array('name' => '秒杀', 'act'=>'store_promotion_spike', 'op'=>'spike_list'),
                array('name' => '闪购', 'act'=>'store_promotion_flash', 'op'=>'flash_list'),
            ...
            'statistics' => array('name' => '统计结算', 'child' => array(
                ...
                array('name' => '门店结算', 'act'=>'store_chain_bill', 'op'=>'index'),
                array('name' => '交易统计', 'act'=>'statistics_trade', 'op'=>'index'),
            )),
            ...
            'account' => array('name' => '账号', 'child' => array(
                ...
                array('name' => '保证金记录', 'act'=>'earnest_money', 'op'=>'index'),
            ))
        );
        if(C('distribute_isuse') == 1) {
            $menu_list['distribute'] = array('name' => '分销管理', 'child' => array(
