...
class store_chain_billControl extends BaseSellerControl {
    ...
    public function indexOp() {
        ...
        if ($_GET['search_type'] == 'chain_name') {
            $condition['ob_chain_name'] = array('like','%'.$_GET['keyword'].'%');
        }
        if ($_GET['search_type'] == 'ob_id') {
            $condition['ob_id'] = intval($_GET['keyword']);
        }
        ...
        Tpl::showpage('store_chain_bill.index');
    }
    ...
    public function show_billOp(){
        ...
        if ($_GET['type'] =='refund'){
            ...
            $sub_tpl_name = 'store_chain_bill.show.refund_list';
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'list':
                $menu_array = array(
                1=>array('menu_key'=>'list','menu_name'=>'门店订单结算', 'menu_url'=>'index.php?act=store_chain_bill&op=list'),
                );
                break;
            case 'show':
                $menu_array = array(
                array('menu_key'=>'order_list','menu_name'=>'订单列表', 'menu_url'=>'index.php?act=store_chain_bill&op=show_bill&ob_id='.$_GET['ob_id']),
                array('menu_key'=>'refund_list','menu_name'=>'退款订单','menu_url'=>'index.php?act=store_chain_bill&op=show_bill&type=refund&ob_id='.$_GET['ob_id'])
                );
                break;
        }
