...
class show_storeControl extends BaseStoreControl {
    ...
    public function indexOp(){
        if(!$this->store_decoration_only) {
            ...
            $new_goods_list = $model_goods->getGoodsOnlineList($condition, $fields, 15, 'goods_id desc', 0, 'goods_commonid', false, $count);

            $condition['goods_commend'] = 1;
            //得到12个推荐商品列表
            $count = $model_goods->getGoodsOnlineCount($condition,"distinct goods_commonid");
            $recommended_goods_list = $model_goods->getGoodsOnlineList($condition, $fields, 15, 'goods_id desc', 0, 'goods_commonid', false, $count);
    ...
    function mansong_goodsOp() {
        $this->_getGoodsList(array('store_id' => $this->store_info['store_id'], 'is_virtual' => 0));
        Tpl::output('page','index');
    ...
    private function _getGoodsList($condition, $order = 'goods_id desc') {
        ...
        $count = $model_goods->getGoodsOnlineCount($condition,"distinct goods_commonid");
        $recommended_goods_list = $model_goods->getGoodsOnlineList($condition, $fields, 15, $order, 0, 'goods_commonid', false, $count);
