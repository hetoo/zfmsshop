...
class buyControl extends BaseBuyControl {
    ...
    public function buy_step1Op() {
        ...
        $result = $logic_buy->buyStep1($_POST['cart_id'], $_POST['ifcart'], $_SESSION['member_id'], $_SESSION['store_id'], $_POST['jjg'],$_POST['ifchain']);
        if (!$result['state']) {
        ...
        Tpl::output('inv_deny', $result['inv_deny']);
        ...
        Tpl::output('order_date_list', $result['order_date_list']);
        Tpl::output('order_date_msg', $result['order_date_msg']);
        if ($_POST['shopnc_cart_list']) {
            Tpl::showpage('buy_store_cart_list','null_layout');
        } else {
            //删除购物车无效商品
    ...
    public function load_chainOp() {
        ...
        if (!empty($stock_list)) {
            ...
            $chain_list = $model_chain->getChainList(array('chain_id' => array('in', $chainid_array),'area_id'=>intval($_GET['area_id']), 'chain_state' => 1, 'is_self_take' => 1),
        ...
        echo $_GET['callback'].'('.json_encode($chain_list).')';
    }
    ...
    public function load_invOp() {
        ...
        if (!empty($list)) {
            foreach ($list as $key => $value) {
               if ($value['inv_state'] == 1) {
                   $list[$key]['content'] = '普通发票'.' '.$value['inv_title'].' '.$value['inv_code'].' '.$value['inv_content'];
               } else {
        ...
        Tpl::output('inv_content_list',$model_inv->getInvContent());
     ...
     public function add_invOp(){
        ...
        if (chksubmit()){
            ...
            if ($_POST['invoice_type'] == 1) {
                ...
                $data['inv_code'] = $_POST['company_code'];
