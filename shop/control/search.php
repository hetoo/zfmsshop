...
class searchControl extends BaseHomeControl {
    ...
    public function storeOp(){
        ...
        //处理排序
        $order = 'is_own_shop desc,store_credit desc,grade_id desc,store_id asc,bind_all_gc desc';
        if (in_array($_GET['key'],array('1','2'))) {
            $sequence = $_GET['order'] == '1' ? 'asc' : 'desc';
            $order = str_replace(array('1','2'), array('store_sales','store_collect'), $_GET['key']);
            $order .= ' '.$sequence;
        }

        // 字段
        $fields = "store_id,store_name,grade_id,member_id,member_name,sc_id,area_info,store_avatar,store_zy,store_credit,store_desccredit,store_servicecredit,store_deliverycredit,store_collect,store_sales,is_own_shop,store_domain";

        $condition = array();
        $condition['store_state'] = 1;
        if ($_GET['keyword'] != '') {
            $condition['store_name|store_zy'] = array('like', '%' . $_GET['keyword'] . '%');
        }
        if (intval($_GET['area_id']) > 0) {
            $condition['province_id'] = intval($_GET['area_id']);
        }

        $store_list = $model_store->getStoreList($condition, 8, $order, $fields);
        ...
        // 地区
        $all_areas = Model('area')->getAreas();
        $region_areas = $all_areas['region'];
        $name_areas = $all_areas['name'];
        Tpl::output('region_areas', $region_areas);
        Tpl::output('name_areas', $name_areas);
