...
class store_activityControl extends BaseSellerControl {
    ...
    public function store_activityOp(){
        ...
        //活动为商品活动，并且为开启状态
        $list   = $activity->getList(array('activity_type'=>'1','store_activity'=>true,'order'=>'activity.activity_sort asc'),$page);
        /**
         * 页面输出
         */
    ...
    public function activity_applyOp(){
        ...
        if(empty($activity_info) || $activity_info['activity_type'] != '1' || $activity_info['activity_state'] != 1 || $activity_info['activity_end_date']<time()){
    ...
    public function activity_apply_saveOp(){
        ...
        //活动类型必须是商品并且活动没有关闭并且活动进行中
        if(empty($activity) || $activity['activity_type'] != '1' || $activity['activity_state'] != '1' || $activity['activity_end_date']<time()){
