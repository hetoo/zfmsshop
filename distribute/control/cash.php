...
class cashControl extends MemberDistributeControl{
    ...
    public function cash_infoOp(){
        ...
        if ($tradc_id <= 0){
            showMessage('参数错误','index.php?act=cash&op=cash_list','html','error');
        }
        $model_tard = Model('dis_trad');
        $condition = array();
        $condition['tradc_member_id'] = $_SESSION['member_id'];
        ...
        if (empty($info)){
            showMessage('记录不存在或已删除','index.php?act=cash&op=cash_list','html','error');
        }
    ...
    public function apply_cashOp(){
        if(chksubmit()){
            ...
            if (floatval($available_trad) < $tradc_amount){
                showDialog('请输入正确的提现金额','index.php?act=cash&op=cash_list','error');
            }
            try {
                ...
                $model_tard->commit();
                showDialog('提现申请成功','index.php?act=cash&op=cash_list','succ');
            } catch (Exception $e) {
                $model_tard->rollback();
                showDialog($e->getMessage(),'index.php?act=cash&op=cash_list','error');
            }
        }
    ...
    private function profile_menu($menu_type,$menu_key=''){
        $menu_array = array(
            array('menu_key'=>'cash_list',        'menu_name'=>'提现记录',    'menu_url'=>'index.php?act=cash&op=cash_list')
        );
        switch ($menu_type) {
            case 'cashinfo':
                ...
                break;
            case 'cashadd':
                $menu_array[] = array('menu_key'=>'cashadd','menu_name'=>'提现申请',    'menu_url'=>'index.php?act=cash&op=apply_cash');
                break;
        }
