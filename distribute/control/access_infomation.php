...
class access_infomationControl extends MemberDistributeControl{
    ...
    public function save_memberOp(){
        if(chksubmit()){
            ...
            if(!$member_info){
                showMessage('账户信息更新失败','index.php?act=access_infomation','','error');
            }else{
                showMessage('账户信息更新成功','index.php?act=access_infomation','','succ');
            }
        }
    }
    ...
    public function distri_quitOp(){
        if($this->member_info['distri_state'] != 2){
            showMessage('您已退出分销或被清退','index.php?act=access_infomation');
        }
    ...
    public function save_quitOp(){
        ...
        if ($stat) {
            ...
            showMessage('退出成功','index.php?act=access_infomation','','succ');
        } else {
            showMessage('退出失败','index.php?act=access_infomation','','error');
        }
    }
    ...
    private function profile_menu($menu_type,$menu_key=''){
        $menu_array = array(
            array('menu_key'=>'access_infomation',        'menu_name'=>'账户设置',    'menu_url'=>'index.php?act=access_infomation&op=member')
        );
        if($menu_type == 'distir_quit'){
            $menu_array[] = array('menu_key'=>'distir_quit',        'menu_name'=>'退出分销',    'menu_url'=>'index.php?act=access_infomation&op=distir_quit');
        }
