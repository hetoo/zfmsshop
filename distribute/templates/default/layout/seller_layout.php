...
<link href="<?php echo DISTRIBUTE_TEMPLATES_URL?>/css/seller_center.css" rel="stylesheet" type="text/css">
...
<!--[if IE 7]>
  ...
  <link rel="stylesheet" href="<?php echo SHOP_RESOURCE_SITE_URL;?>/font/font-awesome3_2/css/font-awesome-ie7.min.css">
<![endif]-->
<script>
...
<script type="text/javascript" src="<?php echo DISTRIBUTE_RESOURCE_SITE_URL;?>/js/seller.js"></script>
...
<header class="ncsc-head-layout w">
  <div class="wrapper">
    ...
    <div class="center-logo"> <a href="<?php echo SHOP_SITE_URL;?>/index.php?act=seller_center&op=index" target="_blank"><img src="<?php echo UPLOAD_SITE_URL.'/'.ATTACH_COMMON.DS.C('seller_center_logo');?>" class="pngFix" alt=""/></a>
      <h1>商家中心</h1>
    </div>
    <div class="index-search-container">
      <div class="index-sitemap"><a href="javascript:void(0);">导航管理 <i class="icon-angle-down"></i></a>
        ...
        <div class="sitemap-menu">
          <div class="title-bar">
            <h2> <i class="icon-sitemap"></i>管理导航</h2>
          ...
          <div id="quicklink_list" class="content">
            ...
            <dl <?php echo ($key == 'promotion' ? 'class="double"' : '');?>>
              ...
              <dd <?php if(!empty($output['seller_quicklink'])) {echo in_array($submenu_value['act'], $output['seller_quicklink'])?'class="selected"':'';}?>>
                <a href="<?php echo $key != 'distribute' ? SHOP_SITE_URL.'/':'';?>index.php?act=<?php echo $submenu_value['act'];?>&op=<?php echo $submenu_value['op'];?>"> <?php echo $submenu_value['name'];?> </a></dd>
              <?php } ?>
              <?php } ?>
            </dl>
            <?php } ?>
      ...
      <div class="search-bar">
        <form method="get" target="_blank" action="<?php echo SHOP_SITE_URL;?>/index.php">
    ...
    <nav class="ncsc-nav">
      <dl class="<?php echo $output['current_menu']['model'] == 'index'?'current':'';?>">
        <dt><a href="<?php echo SHOP_SITE_URL;?>/index.php?act=seller_center&op=index">首页</a></dt>
        <dd class="arrow"></dd>
      </dl>
      <?php if(!empty($output['menu']) && is_array($output['menu'])) {?>
      <?php foreach($output['menu'] as $key => $menu_value) {?>
      <dl class="<?php echo $output['current_menu']['model'] == $key?'current':'';?>">
        <dt><a href="<?php echo $key != 'distribute' ? SHOP_SITE_URL.'/':'';?>index.php?act=<?php echo $menu_value['child'][key($menu_value['child'])]['act'];?>&op=<?php echo $menu_value['child'][key($menu_value['child'])]['op'];?>"><?php echo $menu_value['name'];?></a></dt>
        <dd>
          <ul>
            <?php if(!empty($menu_value['child']) && is_array($menu_value['child'])) {?>
            <?php foreach($menu_value['child'] as $submenu_value) {?>
            <li> <a href="<?php echo $key != 'distribute' ? SHOP_SITE_URL.'/':'';?>index.php?act=<?php echo $submenu_value['act'];?>&op=<?php echo $submenu_value['op'];?>"> <?php echo $submenu_value['name'];?> </a> </li>
            <?php } ?>
            <?php } ?>
          </ul>
        </dd>
