...
<div class="tabmenu">
  ...
  <a href="index.php?act=store_dis_goods&op=goods_list" class="ncbtn ncbtn-mint" title="添加商品"><i class="icon-plus-sign"></i>添加商品</a>
</div>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td class="nscs-table-handle">
        <span><a href="javascript:void(0)" onclick="ajax_get_confirm('您确定要删除吗?', '<?php echo DISTRIBUTE_SITE_URL;?>/index.php?act=store_dis_goods&op=del_goods&id=<?php echo $val['goods_commonid']; ?>');" class="btn-grapefruit"><i class="icon-trash"></i>
        <p>取消分销</p>
        </a></span></td>
    </tr>
    <?php } ?>
...
function change_commis(common_id){
	...
	if (num>0 && num <= 30) {
	    var ajaxurl = 'index.php?act=store_dis_goods&op=edit_goods&num='+num+'&id='+common_id;
		$.ajax({
			type: "GET",
			url: ajaxurl,
			async: false,
