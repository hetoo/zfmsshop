...
<div id="apply_agreement" class="apply-agreement">
  ...
  <div class="bottom"><a id="btn_apply_agreement_next" href="javascript:;" class="btn">下一步，填写分销商认证信息</a></div>
</div>
<script type="text/javascript">
$(document).ready(function(){
    $('#btn_apply_agreement_next').on('click', function() {
        if($('#input_apply_agreement').prop('checked')) {
            window.location.href = "index.php?act=distri_join&op=step1&reup=<?php echo $_GET['reup']?>";
        } else {
            alert('请阅读并同意协议');
        }
    });
