...
class orderModel extends Model {
    private $_order_price_modify = 0;//是否允许店铺修改订单金额
    private $_order_paid_refund = 0;//是否允许店铺取消已付款订单
    ...
    public function setStoreInfo($store_info) {
        $this->_order_price_modify = $store_info['order_price_modify'];
        $this->_order_paid_refund = $store_info['order_paid_refund'];
        return true;
    }
    ...
    public function getOrderInfo($condition = array(), $extend = array(), $fields = '*', $order = '',$group = '') {
        ...
        if (in_array('order_common',$extend)) {
            ...
            $reciver_date_msg = $order_info['extend_order_common']['reciver_date_msg'];
            if (!empty($reciver_date_msg)) $order_info['extend_order_common']['order_message'] = $order_info['extend_order_common']['order_message'].'(希望配送日期：'.$reciver_date_msg.')';
        }
     ...
     */
    public function getStoreOrderList($store_id, $order_sn, $buyer_name, $state_type, $query_start_date, $query_end_date, $skip_off, $fields = '*', $extend = array(),$pagesize = 20) {
        $condition = array();
        ...
        $order_list = $this->getOrderList($condition, $pagesize, $fields, 'order_id desc','', $extend);
        if ($this->_order_paid_refund == 1) $order_list = Model('refund_return')->getGoodsRefundList($order_list,1);//订单商品的退款
        //页面中显示那些操作
        foreach ($order_list as $key => $order_info) {
            ...
            $order_info['if_seller_refund_cancel'] = $this->getOrderOperateState('seller_refund_cancel',$order_info);//订单退款
            //显示发货
    ...
    public function getOrderList($condition, $pagesize = '', $field = '*', $order = 'order_id desc', $limit = '', $extend = array(), $master = false){
        ...
        if (in_array('member',$extend)) {
            $member_list = array();
            foreach ($order_list as $order_id => $order) {
                if (empty($member_list[$order['buyer_id']])) $member_list[$order['buyer_id']] = Model('member')->getMemberInfoByID($order['buyer_id']);
    ...
    public function addOrderLog($data) {
        $data['log_role'] = str_replace(array('buyer','seller','chain','system','admin'),array('买家','商家','门店','系统','管理员'), $data['log_role']);
    ...
    public function getOrderOperateState($operate,$order_info){
        ...
        switch ($operate) {
           ...
           //商家取消订单并退款
           case 'seller_refund_cancel':
               $state = $this->_order_paid_refund == 1 && $order_info['refund'] == 1 && !intval($order_info['lock_state']);
               if ($state && $order_info['order_type'] == 4) {//拼团订单
                $model_pintuan = Model('p_pintuan');
                $_info = $model_pintuan->getOrderInfo(array('order_id'=> $order_info['order_id']));
                if ($_info['end_time'] > TIMESTAMP) $state = false;
           ...
           case 'payment':
            ...
            case 'modify_price':
                $state = ($order_info['order_state'] == ORDER_STATE_NEW) ||
                   ($order_info['payment_code'] == 'offline' && $order_info['order_state'] == ORDER_STATE_PAY);//订单状态
                $state = $order_info['order_type'] != 2 && $order_info['api_pay_time'] == 0 && $state && $order_info['rpt_amount'] == 0;//支付时间及红包
                $state = $this->_order_price_modify == 1 && $order_info['extend_order_common']['voucher_price'] == 0 && $state;//店铺修改权限及代金券
               break;
     ...
     * 秒杀订单数量限制
     */
    public function getSpikeOrderLimit($member_id,$goods_info) {
        $limit = 0;
        $order_limit = $goods_info['order_limit'];
        if ($order_limit > 0) {
            $condition = array();
            ...
            $condition['order_goods.promotions_id'] = $goods_info['promotions_id'];
            $condition['order_state'] = array('in', array(ORDER_STATE_NEW,ORDER_STATE_PAY, ORDER_STATE_SEND, ORDER_STATE_SUCCESS));
            $n = $this->table('order_goods,orders')->join('inner')->on('order_goods.order_id=orders.order_id')->where($condition)->field('orders.add_time')->count();
            if ($n >= $order_limit) $limit = $order_limit;
        }
        return $limit;
    }
}
