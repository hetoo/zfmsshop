...
class goods_browseModel extends Model{
    ...
    public function getViewedGoodsList($member_id = 0,$shownum = 0) {
        ...
        if ($browselist){
            $goods_list_tmp = Model('goods')->getGoodsList(array('goods_id' => array('in', array_keys($browselist))), 'goods_id, goods_name, goods_promotion_price, goods_promotion_type, goods_marketprice, goods_image, store_id, gc_id, gc_id_1, gc_id_2, gc_id_3','','',$shownum);
            $goods_list = array();
     ...
     * 大家都在看
     */
    public function getAllViewedList($page = 10) {
        $result = $this->table('goods_browse')->field('count(DISTINCT goods_id) as count')->find();
        $count = intval($result['count']);
        $list = $this->table('goods_browse')->field('goods_id,max(browsetime) as browsetime')->page($page,$count)->order('browsetime desc')->group('goods_id')->key('goods_id')->select();
        $total_page = $this->gettotalpage();
        if (intval($_GET['curpage'] > $total_page)) return array();
        $goods_list = $this->table('goods')->field('goods_id, goods_name, goods_promotion_price, goods_promotion_type, goods_marketprice, goods_image, store_id')->where(array('goods_id' => array('in', array_keys($list))))->order('goods_id desc')->select();
        return $goods_list;
    }
