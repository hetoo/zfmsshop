...
class cartModel extends Model {
    ...
    private function _addCartDb($goods_info = array(),$quantity) {
        ...
        if (!empty($check_cart)) {
            if(intval($goods_info['goods_storage']) >= $check_cart['goods_num']+$quantity) {
                $this->where($condition)->update(array('goods_num'=> $check_cart['goods_num']+$quantity));
            }
            return true;
        }
    ...
    private function _addCartCookie($goods_info = array(), $quantity = null) {
        ...
        if (in_array($goods_info['goods_id'],array_keys($cart_array))) {
            if(intval($goods_info['goods_storage']) >= $cart_array[$goods_info['goods_id']]['goods_num']+$quantity) {
                $quantity = $cart_array[$goods_info['goods_id']]['goods_num']+$quantity;
            }
        }
    ...
    public function batchAddCart($cart_new_list, $member_id, $store_id=0){
        ...
        if (!empty($goods_list)){
            foreach ($goods_list as $goods_info){
                ...
                if (C('distribute_isuse') == 1 && $goods_info['is_dis'] == 1) {
                    $goods_commonid = $goods_info['goods_commonid'];
                    $dis_member_id = cookie('dis_'.$goods_commonid);
                    if ($dis_member_id) Model('dis_goods')->addDisCart($goods_commonid,$dis_member_id,$member_id);
                }
            }
        }
    }
