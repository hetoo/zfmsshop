...
class p_xianshiModel extends Model{
     ...
     * 过期修改状态
     */
    public function editExpireXianshi($condition = array()) {
        $condition['end_time'] = array('lt', TIMESTAMP);
        $condition['state'] = self::XIANSHI_STATE_NORMAL;
        
        $list = $this->table('p_xianshi')->where($condition)->select();
        if(!empty($list) && is_array($list)) {
            foreach($list as $k => $v) {
                $xianshi_id = $v['xianshi_id'];
                // 更新商品促销价格
                $xianshigoods_list = Model('p_xianshi_goods')->getXianshiGoodsList(array('xianshi_id' => $xianshi_id));
                ...
                $this->editXianshi($update, array('xianshi_id' => $xianshi_id));
            }
        }
        return true;
    }
