...
class predepositModel extends Model {
    ...
    public function changeRcb($type, $data = array())
    {
        ...
        $param['param']['amount'] = ncPriceFormat($amount);
        $param['param']['mp_array'] = Logic('wx_api')->getTemplateData($param['code'],$param['param']);
        // 发送买家消息
    ...
    public function changePd($change_type,$data = array()) {
        ...
        $data_msg['amount'] = ncPriceFormat($data['amount']);
        $param['param'] = $data_msg;
        QueueClient::push('addConsume', array('member_id'=>$data['member_id'],'member_name'=>$data['member_name'],
        'consume_amount'=>$data['amount'],'consume_time'=>TIMESTAMP,'consume_remark'=>$data_log['lg_desc']));
        $param['param']['mp_array'] = Logic('wx_api')->getTemplateData($param['code'],$param['param']);
        RealTimePush('sendMemberMsg', $param);
        return $insert;
    }
