        ...
        if(!empty($code_list) && is_array($code_list)) {
            ...
            switch ($web_page) {
                ...
                case 'channel_fl':
                    ...
                    break;
                case 'index_fl':
                    $style_file = BASE_DATA_PATH.DS.'resource'.DS.'web_config'.DS.'index_fl.php';
                    break;
                default:
                    $style_file = BASE_DATA_PATH.DS.'resource'.DS.'web_config'.DS.'default.php';
                    break;
    ...
    public function getGoodsList($condition = array(),$order = 'goods_id desc',$page = ''){
        ...
        if (!empty($goods_list) && is_array($goods_list)) {
            ...
            if (C('promotion_allow')) {//限时折扣
                $promotion_list = Model('p_time')->getList(array('goods_id' => array('in', $goods_ids), 'start_time' => array('lt', TIMESTAMP), 'end_time' => array('gt', TIMESTAMP)));
                if (!empty($promotion_list) && is_array($promotion_list)) {
                    foreach ($promotion_list as $key => $value) {
                        $goods_id = $value['goods_id'];
                        $goods_price = $value['promotion_price'];
                        $list[$goods_id]['goods_price'] = $goods_price;
                        $list[$goods_id]['goods_type'] = $value['promotion_type']+1;
                    }
                }
            }
