...
class mb_video_focusModel extends Model{

    public function __construct() {
        parent::__construct('mb_video_focus');
    }
    ...
    public function getMbFocusList($condition, $page='', $order='focus_id desc', $field='*') {
        $list = $this->table('mb_video_focus')->field($field)->where($condition)->page($page)->order($order)->select();
        return $list;
    }
    ...
    public function getMbFocusCount($condition) {
        $count = $this->table('mb_video_focus')->where($condition)->count();
        return $count;
    }
    ...
    public function getMbFocusInfoByID($focus_id) {
        ...
        $info = $this->table('mb_video_focus')->where($condition)->find();
        return $info;
    }
    ...
    public function addMbFocus($param){
        return $this->table('mb_video_focus')->insert($param);
    }
    ...
    public function editMbFocus($update, $focus_id) {
        ...
        $condition = array();
        $condition['focus_id'] = $focus_id;
        $result = $this->table('mb_video_focus')->where($condition)->update($update);
        if($result) {
            //删除缓存
    ...
    public function delMbFocusByID($focus_id) {
        ...
        return $this->table('mb_video_focus')->where($condition)->delete();
    }
    ...
    public function isMbFocusItemExist($condition) {
        $item_list = $this->table('mb_video_focus')->where($condition)->select();
        if($item_list) {
            return true;
        } else {
            return false;
    ...
    private function _delMbFocusCache($focus_id) {
        //清理缓存
        dcache($focus_id, 'mb_video_focus');
