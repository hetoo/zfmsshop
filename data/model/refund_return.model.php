...
class refund_returnModel extends Model{
     ...
     * @return int
     */
    public function addRefundReturn($refund_array, $order, $goods = array()) {
        if (!empty($order) && is_array($order)) {
            ...
            $refund_array['chain_id'] = $order['chain_id'];
        }
        if (!empty($goods) && is_array($goods)) {
            ...
            $refund_array['is_dis'] = $goods['is_dis'];
            $refund_array['dis_commis_rate'] = $goods['dis_commis_rate'];
        ...
        if ($refund_array['seller_state'] == 1) RealTimePush('sendStoreMsg', $param);
        ...
        if ($refund_id > 0) {
            ...
            try {
                ...
                if ($state) {
                    ...
                    if ($state && $order_state == $order_completed) {//已收货时更新完成时间（结算使用）
                        $refund_array = array();
    ...
    public function getGoodsRefundList($order_list = array(), $order_refund = 0) {
        ...
        if (empty($order_ids)) return $order_list;
        $model_trade = Model('trade');
        $condition = array();
