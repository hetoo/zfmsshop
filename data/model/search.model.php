...
//                 showMessage($e->getMessage(),'','html','error');
    ...
    private function _setQueryXS($condition,$order) {
        ...
        if (isset($condition['is_own_shop'])) {
            $this->_xs_search->addQueryString('is_own_shop'.':'.$condition['is_own_shop']);
        }
        ...
        if (count($order) > 1) {
            $this->_xs_search->addWeight('is_own_shop', '1',5);
            if (empty($condition['keyword'])) $this->_xs_search->setCollapse('goods_commonid_colorid')->setMultiSort($order);
        } else {
    ...
    private function _searchXS(){
        ...
        $brands = array();
        $cates = array();
        $goods_v = array('goods_id','goods_name','goods_jingle','gc_name','brand_name','brand_id','is_book','attr_id',
                    'store_id','area_id','have_gift','goods_promotion_price','goods_click','goods_salenum','goods_barcode','is_own_shop');
        foreach ($docs as $doc) {
            foreach ($goods_v as $v) {
   ...
   public function getTagCategory($keyword = '') {
    ...
    public function indexerSearch($get = array(), $pagesize) {
        ...
        //拼接排序(销量,浏览量,价格)
        $order = array();
        $order = array('is_own_shop' => false,'goods_id' => false);
