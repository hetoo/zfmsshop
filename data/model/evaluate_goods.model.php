...
class evaluate_goodsModel extends Model {
     ...
     * 根据商品编号查询商品评价信息
     */
    public function getEvaluateGoodsInfoByGoodsID($goods_id,$goods_info) {
        $info = array();
        $info['img'] = 0;
        $count_array = $this->field('count(*) as count,geval_scores')->where(array('geval_goodsid' => $goods_id))->group('geval_scores')->limit(5)->key(geval_scores)->select();
        ...
        if(intval($info['all']) > 0) {
            ...
            if(intval($info['all']) > $goods_info['evaluation_count']) {
                //更新商品表好评星级和评论数
