...
class p_mansong_ruleModel extends Model{
    ...
    public function getMansongRuleListByID($mansong_id) {
        ...
        $mansong_rule_list = $this->where($condition)->order('price asc')->limit(3)->select();
        if(!empty($mansong_rule_list)) {
            ...
            for($i =0, $j = count($mansong_rule_list); $i < $j; $i++) {
                ...
                if(!empty($goods_id)) {
                    $goods_info = $model_goods->getGoodsOnlineInfoByID($goods_id,"goods_id,goods_name,store_id,goods_image,goods_storage");
                    if(!empty($goods_info)) {
