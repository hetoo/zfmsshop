...
class dis_web_configModel extends Model{
    /**
     * 读取模块内容记录
     *
     * @param
    ...
    public function getCodeRow($code_id,$web_id){
        ...
        $param['web_id']    = $web_id;
        $result = $this->table('dis_web_code')->where($param)->find();
        return $result;
    }
    ...
    public function getCodeList($condition = array()){
        $result = $this->table('dis_web_code')->where($condition)->order('web_id')->select();
        return $result;
    }
    ...
    public function updateCode($condition,$data){
        ...
        if (is_array($data)){
            $result = $this->table('dis_web_code')->where($condition)->update($data);
            return $result;
        } else {
            return false;
        }
    ...
    public function getWebList($condition = array('web_page' => 'index'),$page = '',$order = 'web_sort'){
        $result = $this->table('dis_web')->where($condition)->order($order)->page($page)->select();
        return $result;
    }
    ...
    public function updateWeb($condition,$data){
        ...
        if (is_array($data)){
            $result = $this->table('dis_web')->where($condition)->update($data);
            return $result;
        } else {
            return false;
        }
    ...
    public function updateWebHtml($web_id = 1,$web_array = array()){
        ...
        if(!empty($code_list) && is_array($code_list)) {
            ...
            switch ($web_page) {
                case 'index':
                    $style_file = BASE_DATA_PATH.DS.'resource'.DS.'distri_web_config'.DS.'default.php';
                    break;
                case 'index_pic':
                    $style_file = BASE_DATA_PATH.DS.'resource'.DS.'distri_web_config'.DS.'focus.php';
                    break;
                case 'index_right_pic':
                    $style_file = BASE_DATA_PATH.DS.'resource'.DS.'distri_web_config'.DS.'right.php';
                    break;
                default:
                    $style_file = BASE_DATA_PATH.DS.'resource'.DS.'distri_web_config'.DS.'default.php';
                    break;
