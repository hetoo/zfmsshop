...
class cacheModel extends Model {
     ...
     * 敏感词库缓存
     */
    private function _sensitive_word() {
        $list = $this->table('sensitive_word')->field('word_name')->where(array('is_open' => 1))->limit(false)->select();
        if (!is_array($list)) return array();
        $list_new = array();
        if(!empty($list)){
            foreach ($list as $value){
                array_push($list_new, $value['word_name']);
            }
        }
        return $list_new;
    }
