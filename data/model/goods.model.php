...
class goodsModel extends Model{
    ...
    public function getGoodsCommonList($condition, $field = '*', $page = 10, $order = 'goods_commonid desc', $limit = '') {
        $condition = $this->_getRecursiveClass($condition);
        if (!empty($condition['storage']) && is_array($condition['storage'])) {//查询总库存时用连查分组
            $storage = $condition['storage'];
            unset($condition['storage']);
            $this->db->subquery = 'count(*) as count';
            $list_count = $this->table('goods_common,goods')->join('inner')->on('goods_common.goods_commonid=goods.goods_commonid')
                ->where($condition)->having("sum(goods.goods_storage)>=".$storage[0].' and sum(goods.goods_storage)<='.$storage[1])->group('goods_common.goods_commonid')->field('goods_common.goods_commonid')->limit(false)->select();
            $goods_commend_list = array();
            if (intval($list_count[0]['count']) > 0) {
                $goods_commend_list = $this->table('goods_common,goods')->join('inner')->on('goods_common.goods_commonid=goods.goods_commonid')->field('goods_common.*,sum(goods.goods_storage) as storage')
                    ->where($condition)->having("storage>=".$storage[0].' and storage<='.$storage[1])->group('goods_common.goods_commonid')->order($order)->page($page, intval($list_count[0]['count']))->select();
                $this->cls();
            }
     ...
     * @return array
     */
    public function getGoodsCommonOnlineList($condition, $field = '*', $page = 10, $order = "goods_common.goods_commonid desc") {
    ...
    public function getGoodsCommonLockUpList($condition, $field = '*', $page = 10, $order = "goods_commonid desc", $limit = '') {
        $condition['goods_common.goods_state']   = self::STATE10;
        $condition['goods_common.goods_verify']  = self::VERIFY1;
    ...
    public function getGoodsCommonWaitVerifyList($condition, $field = '*', $page = 10, $order = "goods_commonid desc", $limit = '') {
        if (!isset($condition['goods_common.goods_verify'])) {
            $condition['goods_common.goods_verify']  = array('neq', self::VERIFY1);
        }
        return $this->getGoodsCommonList($condition, $field, $page, $order, $limit);
    }
    ...
    public function getGoodsCommendList($store_id, $limit = 5) {
        if (C('dbdriver') == 'mysql') {
            $goods_commend_list = $this->getGoodsOnlineList(array('store_id' => $store_id, 'goods_commend' => 1, 'is_book' => 0), 'goods_id,goods_name,goods_jingle,goods_image,store_id,goods_promotion_price,goods_salenum,goods_collect', 0, 'rand()', $limit, 'goods_commonid');
        } elseif (C('dbdriver') == 'oracle') {
            $goods_id_list = $this->getGoodsOnlineList(array('store_id' => $store_id, 'goods_commend' => 1, 'is_book' => 0), 'min(goods_id) as goods_id', 0, 'rand()', $limit,'goods_commonid');
            if (!empty($goods_id_list)) {
                $tmp = array();
                foreach ($goods_id_list as $v) {
                    $tmp[] = $v['goods_id'];
                }
                $goods_commend_list = $this->getGoodsOnlineList(array('goods_id' => array('in',$tmp)), 'goods_id,goods_name,goods_jingle,goods_image,store_id,goods_promotion_price,goods_salenum,goods_collect', 0, 'rand()', $limit);
            }
        }
        return $goods_commend_list;
    }
    ...
    public function editGoodsById($update, $goodsid_array, $updateXS = false) {
        ...
        if (!is_array($goodsid_array)) $condition['goods_id'] = $goodsid_array;
     ...
     * @return boolean
     */
    public function editGoodsPromotionPrice($condition, $promotion_goods = array()) {
        ...
        foreach ($goods_array as $key => $val) {
            ...
            foreach ($val as $k => $v) {
                ...
                // 查询价格促销是否进行
                if (empty($promotion_goods)) $promotion_goods = $model_p_time->getInfo(array('goods_id' => $k, 'start_time' => array('lt', TIMESTAMP), 'end_time' => array('gt', TIMESTAMP)));
                if (!empty($promotion_goods)) {
                    // 更新价格
                    $this->editGoodsById(array('goods_promotion_price' => $promotion_goods['promotion_price'], 'goods_promotion_type' => $promotion_goods['promotion_type']), $k);
                    continue;
                }
    ...
    public function editGoodsCommonById($update, $commonid_array) {
        ...
        if (!is_array($commonid_array)) $condition['goods_commonid'] = $commonid_array;
    ...
    public function getGoodsInfoAndPromotionById($goods_id) {
        ...
        if($goods_info['is_batch'] == 0){//非批发商品查促销
            // 手机专享
            ...
            if (C('promotion_allow') && empty($goods_info['sole_info']) && empty($goods_info['groupbuy_info'])) {
                $promotion_goods = Model('p_time')->getGoodsInfo($goods_info['goods_id']);
                $goods_info['promotion_info'] = $promotion_goods;
                if ($promotion_goods['promotion_type']==2) $goods_info['xianshi_info'] = $promotion_goods;
            }
    ...
    public function delGoods($condition) {
        ...
        if (!empty($goods_list)) {
            ...
            Model('p_spike_goods')->delSpikeGoods(array('goods_id' => array('in', $goodsid_array)));
            Model('p_flash_goods')->delFlashGoods(array('goods_id' => array('in', $goodsid_array)));
            //删除商品浏览记录
            ...
            Model('chain_cart')->delChainCart(array('goods_id' => array('in', $goodsid_array)));
            Model('cart')->delCart('db',array('goods_id' => array('in', $goodsid_array)));
        }
    ...
    private function _sendStoreMsg($code, $store_id, $param) {
        RealTimePush('sendStoreMsg', array('code' => $code, 'store_id' => $store_id, 'param' => $param));
    }
    ...
    public function getVirtualGoodsOnlineInfoByID($goods_id) {
        $goods_info = $this->getGoodsInfoByID($goods_id,'*');
        return $goods_info['is_virtual'] == 1 && $goods_info['goods_state'] == self::STATE1 && $goods_info['virtual_indate'] >= TIMESTAMP ? $goods_info : array();
    }
    ...
    public function getGoodsSpecListByCommonId($common_id) {
            $spec_array = $this->getGoodsList(array('goods_commonid' => $common_id), 'goods_spec,goods_id,store_id,goods_image,color_id','','',99);
        return $spec_array;
    }
    ...
    public function getGoodsDetail($goods_id) {
        ...
        //视频
        if($goods_info['goods_video']){
            $video_path = goodsVideoPath($goods_info['goods_video'], $goods_info['store_id']);
        }
        ...
        if (!empty($image_more)) {
            ...
            $image_list[] = array( '_small' => thumb($goods_info,60) , '_mid' => thumb($goods_info , 360) ,'_big' => thumb($goods_info , 1280) );
        ...
        if ($goods_info['is_book'] != '1') {
            if (!empty($goods_info['promotion_info'])) {
                if ($goods_info['promotion_info']['promotion_type']==2) {//类型:2限时折扣,3秒杀,4闪购
                    $goods_info['promotion_type'] = 'xianshi';
                    $goods_info['title'] = $goods_info['xianshi_info']['xianshi_title'];
                    $goods_info['remark'] = $goods_info['xianshi_info']['xianshi_explain'];
                    $goods_info['promotion_price'] = $goods_info['xianshi_info']['xianshi_price'];
                ...
                if ($goods_info['promotion_info']['promotion_type']==3) {
                    $goods_info['promotion_type'] = 'spike';
                    $goods_info['title'] = $goods_info['promotion_info']['spike_title'];
                    $goods_info['remark'] = $goods_info['promotion_info']['spike_explain'];
                    $goods_info['promotion_price'] = $goods_info['promotion_info']['spike_price'];
                    $goods_info['down_price'] = ncPriceFormat($goods_info['goods_price'] - $goods_info['promotion_info']['spike_price']);
                    $goods_info['upper_limit'] = $goods_info['promotion_info']['upper_limit'];
                    $goods_info['explain'] = $goods_info['promotion_info']['spike_explain'];
                }
                if ($goods_info['promotion_info']['promotion_type']==4) {
                    $goods_info['promotion_type'] = 'flash';
                    $goods_info['title'] = $goods_info['promotion_info']['flash_title'];
                    $goods_info['remark'] = $goods_info['promotion_info']['flash_explain'];
                    $goods_info['promotion_price'] = $goods_info['promotion_info']['flash_price'];
                    $goods_info['down_price'] = ncPriceFormat($goods_info['goods_price'] - $goods_info['promotion_info']['flash_price']);
                    $goods_info['upper_limit'] = $goods_info['promotion_info']['upper_limit'];
                    $goods_info['explain'] = $goods_info['promotion_info']['flash_explain'];
                }
                unset($goods_info['promotion_info']);
            }
            //团购
            ...
            // 验证是否允许送赠品
            if ($this->checkGoodsIfAllowGift($goods_info) && $goods_info['have_gift'] == 1) {
        ...
        if($goods_info['is_batch'] == 1){
            $batch_price = (array)unserialize($goods_info['batch_price']);
            $batch_list = array_keys($batch_price);
            $batch_count = count($batch_price);
            $i = 1;
            $batch_show_list = array();
            foreach ($batch_price as $key => $val) {
                $tmp = array();
                if($batch_count == $i){
                    $tmp['batch'] = '≥'.$key;
                    $tmp['price'] = $val;
                }else{
                    $start_num = intval($batch_list[$i-1]);
                    $end_num = intval($batch_list[$i]-1);
                    $tmp['batch'] = $start_num.'-'.$end_num;
                    $tmp['price'] = $val;
                }
                $i++;
                $batch_show_list[] = $tmp;
            }
            $goods_info['batch_price'] = $batch_show_list;
        }

        $this->editGoodsById(array('goods_click' => array('exp', 'goods_click + 1')), $goods_id);
        $this->editGoodsCommonById(array('click_count' => array('exp', 'click_count + 1')),$goods_info['goods_commonid']);
        $result = array();
