...
class buyLogic {
     ...
     */
    public function buyStep1($cart_id, $ifcart, $member_id, $store_id, $jjg = null,$ifchain = null) {
        //得到购买商品信息
        if ($ifcart) {
            $result = $this->getCartList($cart_id, $member_id, $jjg);
        } else {
     ...
     */
    public function getCartList($cart_id, $member_id, $jjg = null) {
    ...
    public function getGoodsList($cart_id, $member_id, $store_id) {
        ...
        if($goods_info['promotion_info']['promotion_type']==3) {
            $_n = Model('order')->getSpikeOrderLimit($member_id,$goods_info);
            if($_n > 0) {
                return callback(false, '抱歉，秒杀商品只能购买'.$_n.'次，请重新选择');
            }
        }
        ...
        if ($_POST['pintuan'] == 1) {
            ...
            if (!empty($pintuan_info) && is_array($pintuan_info)) {
                ...
                $goods_info['goods_price'] = $pintuan_info['pintuan_price'];
            }
        }
    ...
    public function getBuyStep1Data($member_id, $data) {
        ...
        if (!empty($pay_goods_list['offline'])) {
            ...
            if ($_POST['pintuan'] == 1) $result['ifshow_offpay'] = false;
        } else {
        ...
        } elseif (!empty($inv_info) && !$inv_deny) {
        ...
        $order_date_content = C('order_date_content');
        $order_date_array = explode("\n", $order_date_content);
        if (empty($order_date_array) || !is_array($order_date_array)) {
            $order_date_array = array();
        }
        $result['order_date_list'] = $order_date_array;
        $result['order_date_msg'] = C('order_date_msg');
    ...
    private function _createOrderStep1() {
        ...
        if (!$post['chain']['id']) {
            ...
            $chain_info = Model('chain')->getChainInfo(array('chain_id'=>intval($post['chain']['id']), 'chain_state' => 1, 'is_self_take' => 1));
            if ($chain_info) {
            ...
            foreach ($post['voucher'] as $store_id => $voucher) {
                if (preg_match_all('/^(\d+)\|(\d+)\|([\d.]+)$/',$voucher,$matchs)) {
                    if (floatval($matchs[3][0]) > 0) {
                        if ($input_chain_id) $store_id = $chain_info['store_id'];
                        $input_voucher_list[$store_id]['voucher_t_id'] = $matchs[1][0];
                        $input_voucher_list[$store_id]['voucher_price'] = 0;
                        $_info = array();
                        if (!$post['chain']['id']) {
                            $_info = Model('voucher')->table('voucher')->where(array('voucher_t_id'=> $matchs[1][0],'voucher_owner_id'=> $this->_member_info['member_id'],'voucher_state'=> 1))->master(true)->lock(true)->find();//锁定当前代金券记录
                        } else {
        ...
        if ($post['rpt']) {
            if (preg_match_all('/^(\d+)\|([\d.]+)$/',$post['rpt'],$matchs)) {
                if (floatval($matchs[2][0]) > 0) {
                    ...
                    $input_rpt_info['rpacket_price'] = 0;
                    $_info = Model('redpacket')->table('redpacket')->where(array('rpacket_t_id'=> $matchs[1][0],'rpacket_owner_id'=> $this->_member_info['member_id'],'rpacket_state'=> 1))->master(true)->lock(true)->find();//锁定当前红包记录
                    if ($_info['rpacket_state'] != 1) {
                        throw new Exception('请选择正确的红包');
                    }
                    $input_rpt_info['rpacket_price'] = $_info['rpacket_price'];
                }
            }
        }
        ...
        if ($post['ifcart']) {
            ...
            if ($_POST['pintuan'] == 1) {
                ...
                if (!empty($pintuan_info) && is_array($pintuan_info)) {
                    $goods_info['pintuan_promotion'] = '1';
                    $goods_info['goods_price'] = $pintuan_info['pintuan_price'];
                    $goods_info['pintuan_info'] = $pintuan_info;
                }
            }
    ...
    private function _createOrderStep4() {
        ...
        if (C('distribute_isuse') == 1) {//分销商品
            ...
            $dis_cart_list = $model_dis_goods->getDisCartList($member_id);
        }
...
//             $should_goods_total = $store_final_order_total[$store_id]-$store_freight_total[$store_id]+$promotion_total;
            ...
            if (isset($input_voucher_list[$store_id])){
                ...
                $order_common['promotion_info'][] = array($_voucher_type,sprintf('使用%s元代金券 编码：%s',$input_voucher_list[$store_id]['voucher_price'],$input_voucher_list[$store_id]['voucher_code']));
            }
            ...
            foreach ($goods_list as $goods_info) {
                ...
                if (!intval($goods_info['bl_id'])) {
                    ...
                    if ($goods_info['ifgroupbuy']) {
                        ...
                        if ($goods_info['is_member_price'] == 1) $order_goods[$i]['goods_type'] = 12;//会员价促销级别最低
                    }
                    if ($goods_info['pintuan_promotion']) $order_goods[$i]['goods_type'] = 10;
                    if ($input_chain_id && !$goods_info['ifzengpin']) $order_goods[$i]['goods_type'] = 1;
                    ...
                    //计算本件商品优惠金额
                    $promotion_value = floor($goods_total*($promotion_rate)*10)/10;//优惠金额精确到1角
                    ...
                    $dis_member_id = cookie('dis_'.$goods_commonid) ? cookie('dis_'.$goods_commonid) : $dis_cart_list[$goods_commonid]['dis_member_id'];
                    if (C('distribute_isuse') == 1 && $goods_info['is_dis'] == 1 && $dis_member_id > 0 && $order_goods[$i]['goods_pay_price'] >= 1) {//分销商品佣金比例
                        $order_is_dis = 1;
                        $order_goods[$i]['is_dis'] = 1;
                        $order_goods[$i]['dis_member_id'] = $dis_member_id;
                        $_info = $model_goods->getGoodsCommonInfo(array('goods_commonid'=> $goods_commonid));
                        $order_goods[$i]['dis_commis_rate'] = $_info['dis_commis_rate'];
                        setNcCookie('dis_' . $goods_commonid, 0, -1);
                        Model('dis_goods')->delDisCart($goods_commonid, $member_id);
                    }
                    $i++;
                    ...
                    foreach ($goods_info['bl_goods_list'] as $bl_goods_info) {
                        $goods_commonid = $bl_goods_info['goods_commonid'];
                        $order_goods[$i]['order_id'] = $order_id;
                        ...
                        //计算本件商品优惠金额
                        $promotion_value = floor($goods_total*($promotion_rate)*10)/10;
                        $order_goods[$i]['goods_pay_price'] = $goods_total - $promotion_value < 0 ? 0 : $goods_total - $promotion_value;
                        $promotion_sum += $promotion_value;
                        $order_goods[$i]['is_dis'] = 0;
                        $order_goods[$i]['dis_member_id'] = 0;
                        $order_goods[$i]['dis_commis_rate'] = 0;
                        $dis_member_id = cookie('dis_'.$goods_commonid) ? cookie('dis_'.$goods_commonid) : $dis_cart_list[$goods_commonid]['dis_member_id'];
                        if (C('distribute_isuse') == 1 && $bl_goods_info['is_dis'] == 1 && $dis_member_id > 0 && $order_goods[$i]['goods_pay_price'] >= 1) {//分销商品佣金比例
                            $order_is_dis = 1;
                            $order_goods[$i]['is_dis'] = 1;
                            $order_goods[$i]['dis_member_id'] = $dis_member_id;
                            $_info = $model_goods->getGoodsCommonInfo(array('goods_commonid'=> $goods_commonid));
            ...
            if ($order_is_dis) {
                $model_order->editOrder(array('is_dis'=> $order_is_dis),array('order_id'=> $order_id));
            }
            ...
            if ($order['order_type'] == 4) {//拼团订单
                ...
                $_array['min_num'] = $goods_list[0]['pintuan_info']['min_num'];
                $pintuan_end_time = $goods_list[0]['pintuan_info']['end_time'];
                $_end = TIMESTAMP+60*60*24;
                if ($_end > $pintuan_end_time) $_end = $pintuan_end_time;
                if ($_POST['log_id']) {
                    $condition = array();
    ...
    private function _createOrderStep6() {
        ...
        //变更库存和销量
        $result = Logic('queue')->createOrderUpdateStorage($goods_buy_quantity, $goods_sale);
        if (!$result['state']) {
        ...
        if (!empty($input_rpt_info) && is_array($input_rpt_info)) {
            $result = Logic('realtime_msg')->editRptState($input_rpt_info,$pay_sn);
            if (!$result['state']) {
        ...
        if ($input_dlyp_id) {
            ...
            $data['chain_price'] = $input_address_info['chain_price'];
        ...
        if (!empty($notice_list)) {
            foreach ($notice_list as $code => $value) {
                RealTimePush('sendStoreMsg', array('code' => $code, 'store_id' => key($value), 'param' => current($value)));
            }
        }
