...
class buy_1Logic {
    ...
    public function getGoodsCartList($cart_list, \StdClass $jjgObj = null) {
        ...
        //价格促销
        $this->getPromotionCartList($cart_list);
    ...
    public function getGoodsOnlineInfo($goods_id,$quantity) {
        ...
        if (!$goods_info['is_book']) {
            ...
            //价格促销
            $this->getPromotionInfo($goods_info,$goods_info['goods_num']);
     ...
     * @return array
     */
    public function getChainAvailableVoucherList($store_goods_total, $member_id, $chain_id) {
        $voucher_list = array();
     ...
     * @return array
     */
    public function reChainParseVoucherList($input_voucher_list = array(), $store_goods_total = array(), $member_id, $chain_id) {
        if (empty($input_voucher_list) || !is_array($input_voucher_list)) return array();
        $store_voucher_list = $this->getChainAvailableVoucherList($store_goods_total, $member_id, $chain_id);
        foreach ($input_voucher_list as $store_id => $voucher) {
            $tmp = $store_voucher_list[$store_id];
            if (is_array($tmp) && isset($tmp[$voucher['voucher_t_id']])) {
                $input_voucher_list[$store_id]['voucher_id'] = $tmp[$voucher['voucher_t_id']]['voucher_id'];
     ...
     */
    public function getPromotionInfo( & $goods_info, $quantity) {
        if (empty($quantity)) $quantity = 1;
        if (!C('promotion_allow') || empty($goods_info['promotion_info']) || !empty($goods_info['groupbuy_info']) || $goods_info['ifsole']) return ;
        $promotion_goods = $goods_info['promotion_info'];
        if ($promotion_goods['promotion_type']==2) {//限时折扣中，如果购买数量若>=规定的下限，按折扣价格计算,否则按原价计算
            $goods_info['xianshi_info']['down_price'] = ncPriceFormat($goods_info['goods_price'] - $promotion_goods['promotion_price']);
            $goods_info['promotion_info']['down_price'] = $goods_info['xianshi_info']['down_price'];
            if ($quantity >= $promotion_goods['lower_limit']) {
                $goods_info['goods_yprice'] = $goods_info['goods_price'];
                $goods_info['goods_price'] = $promotion_goods['promotion_price'];
                $goods_info['promotions_id'] = $promotion_goods['promotion_id'];
                $goods_info['ifxianshi'] = true;
                $goods_info['ifpromotion'] = true;
            }
        } else {//秒杀、闪购
            $goods_info['promotion_info']['down_price'] = ncPriceFormat($goods_info['goods_price'] - $promotion_goods['promotion_price']);
            if ($promotion_goods['upper_limit']==0 || $quantity <= $promotion_goods['upper_limit']) {
                $goods_info['goods_yprice'] = $goods_info['goods_price'];
                $goods_info['goods_price'] = $promotion_goods['promotion_price'];
                $goods_info['promotions_id'] = $promotion_goods['promotion_id'];
                $goods_info['ifpromotion'] = true;
                if ($promotion_goods['order_limit']>0) $goods_info['order_limit'] = $promotion_goods['order_limit'];
            }
        }
    ...
    public function getReciverAddr($address_info = array()) {
        if (intval($address_info['dlyp_id'])) {
            ...
            $reciver_info['chain_price'] = $address_info['chain_price'];
            $reciver_info['dlyp'] = 1;
    ...
    public function createInvoiceData($invoice_info){
        ...
        if ($invoice_info['inv_state'] == 1) {
            ...
            if (!empty($invoice_info['inv_code'])) $inv['纳税人识别号'] = $invoice_info['inv_code'];
    ...
    public function extendInPay($order_list) {
        ...
        if ($order_list[0]['order_type'] == 4) {//拼团订单
            ...
            $model_pintuan->payOrder($order_list[0]);
        }
        return callback(true);
    }
    ...
    private function _getGoodsOnlineInfo($goods_id,$quantity) {
        ...
        $goods_info = $this->_getMemberPrice($goods_info);//会员价促销级别最低
        $new_array = array();
        ...
        $new_array['goods_price'] = $this->_getGoodsBatchPrice($goods_info,$quantity);
        ...
        $new_array['goods_inv'] = $goods_info['goods_inv'];
        $new_array['goods_vat'] = $goods_info['goods_vat'];
        $new_array['goods_voucher'] = $goods_info['goods_voucher'];
        ...
        $new_array['promotion_info'] = $goods_info['promotion_info'];
        $new_array['is_member_price'] = $goods_info['is_member_price'];
        $new_array['is_chain'] = $goods_info['is_chain'];
        $new_array['is_dis'] = $goods_info['is_dis'];
     ...
     * 批发商品的销售价格
     *
     * @param int $goods_info 所购商品
     * @param int $quantity 购买数量
     * @return array
     */
    private function _getGoodsBatchPrice($goods_info,$quantity) {
        if ($goods_info['is_batch'] == 1) {
            ...
            foreach ($batch_price as $k => $v) {
                if ($k <= $quantity && $k <= $goods_storage) {
                    $goods_info['goods_price'] = ncPriceFormat($v);
                }
            }
        }
     ...
     * @return array
     */
    private function _getMemberPrice($goods_info) {
        $goods_info = Logic('goods')->getGoodsMemberPrice($goods_info, $this->member_info);
        if ($goods_info['is_member_price'] == 1 && $goods_info['member_price'] > 0) {
            $goods_info['goods_price'] = ncPriceFormat($goods_info['member_price']);
        } else {
    ...
    private function _getOnlineCartList($cart_list) {
        ...
        foreach ((array)$cart_list as $key => $cart_info) {
            ...
            if (in_array($cart_info['goods_id'],array_keys($goods_online_array))) {
                ...
                $goods_online_info = $this->_getMemberPrice($goods_online_info);//会员价促销级别最低
                ...
                $cart_list[$key]['goods_price'] = $this->_getGoodsBatchPrice($goods_online_info,$cart_info['goods_num']);
                ...
                $cart_list[$key]['goods_inv'] = $goods_online_info['goods_inv'];
                $cart_list[$key]['goods_vat'] = $goods_online_info['goods_vat'];
                $cart_list[$key]['goods_voucher'] = $goods_online_info['goods_voucher'];
                ...
                $cart_list[$key]['promotion_info'] = $goods_online_info['promotion_info'];
                $cart_list[$key]['is_chain'] = $goods_online_info['is_chain'];
                $cart_list[$key]['is_member_price'] = $goods_online_info['is_member_price'];
                $cart_list[$key]['is_dis'] = $goods_online_info['is_dis'];
    ...
    /**
     * 批量判断购物车内的商品是不是价格促销中，如果购买数量若>=规定的下限，按折扣价格计算,否则按原价计算
     * 并标识该商品为限时商品
     * @param array $cart_list
     */
    public function getPromotionCartList(& $cart_list) {
        if (!C('promotion_allow') || empty($cart_list)) return ;
        foreach ($cart_list as $key => $cart_info) {
            if (intval($cart_info['bl_id']) || empty($cart_info['promotion_info']) || !empty($cart_info['groupbuy_info']) || $cart_info['ifsole']) continue;
            $this->getPromotionInfo($cart_info, $cart_info['goods_num']);
            $cart_list[$key] = $cart_info;
        }
    }
        ...
        foreach ($cart_list as $key => $cart_info) {
            ...
            foreach ($cart_list[$key]['bl_goods_list'] as $k => $goods_info) {
                if (array_key_exists($goods_info['goods_id'],$goods_online_list)) {
                    ...
                    $cart_list[$key]['bl_goods_list'][$k]['is_dis'] = $goods_online_info['is_dis'];
                    $cart_list[$key]['bl_goods_list'][$k]['dis_commis_rate'] = $goods_online_info['dis_commis_rate'];
                    $cart_list[$key]['bl_goods_list'][$k]['goods_inv'] = $goods_online_info['goods_inv'];
                    $cart_list[$key]['bl_goods_list'][$k]['goods_vat'] = $goods_online_info['goods_vat'];
                    $cart_list[$key]['bl_goods_list'][$k]['goods_voucher'] = $goods_online_info['goods_voucher'];
            ...
            foreach ($store_cart as $cart_info) {
                if (!intval($cart_info['bl_id'])) {
                    ...
                    foreach ($cart_info['bl_goods_list'] as $goods_info) {
                        ...
                        $goods_sale[$goods_info['goods_id']] = $goods_info['goods_commonid'];
                    }
                }
            }
        }
