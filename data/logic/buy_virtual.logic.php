...
class buy_virtualLogic {
    ...
    public function buyStep3($post, $member_id) {
        try {
            ...
            $_info = $model_goods->table('goods')->where(array('goods_id'=> $post['goods_id']))->master(true)->lock(true)->find();//锁定记录
    ...
    private function _createOrder($input, $goods_info, $member_info) {
        ...
        if ($goods_info['goods_storage_alarm'] >= ($goods_info['goods_storage'] - $input['quantity'])) {
            ...
            RealTimePush('sendStoreMsg', array('code' => 'goods_storage_alarm', 'store_id' => $goods_info['store_id'], 'param' => $param));
        }
