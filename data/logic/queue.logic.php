...
class queueLogic {
    ...
    public function createOrderUpdateChainStorage($goods_buy_quantity,$chain_id) {
        ...
        if (!$result) {
            return callback(false,'变更门店商品库存失败');
        } else {
            return callback(true);
        }
    }
    ...
    public function cancelOrderUpdateChainStorage($goods_buy_quantity,$chain_id) {
        ...
        foreach ($goods_buy_quantity as $goods_id => $quantity) {
            ...
            $condition['goods_salenum'] = array('egt',$quantity);
            $condition['goods_id'] = $goods_id;
            $result = $model_chain->editChainStock($data, $condition);
            if (!$result) {
                break;
        ...
        if (!$result) {
            return callback(false,'变更门店商品库存与销量失败');
        } else {
            return callback(true);
        }
    }
    ...
    public function saveDeliveryOrder($param) {
        ...
        foreach ($param['order_sn_list'] as $order_id => $v) {
            ...
            $data['chain_id'] = $param['dlyp_id'];
            $data['chain_price'] = $param['chain_price'];
            $data['reciver_name'] = $param['reciver_name'];
            $data['reciver_telphone'] = $param['tel_phone'];
            $data['reciver_mobphone'] = $param['mob_phone'];
            $insert = $model_chain_order->addDeliveryOrder($data);
            if (!$insert) {
                return callback(false,'保存门店代收订单信息失败order_sn:'.$v['order_sn']);
            }
        }
        return callback(true);
    }
    ...
    public function createSphot($order_id_list = array()) {
        $goods_list = Model('order')->getOrderGoodsList(array('order_id'=>array('in',$order_id_list)),'rec_id,goods_id,goods_type,gc_id');
        if ($goods_list) {
            $model_sp = Model('order_snapshot');
            foreach ($goods_list as $goods_info) {
                $model_sp->createSphot($goods_info);
            }
        }
        return callback(true);
    }
    ...
    public function createVrSphot($order_info = array()) {
        Model('vr_order_snapshot')->createSphot($order_info['order_id'], $order_info['goods_id']);
        return callback(true);
    }
