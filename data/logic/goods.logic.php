...
class goodsLogic
{
        ...
        if (is_array($param['spec'])) {
            foreach ($param['spec'] as $value) {
                ...
                if (!empty($goods_info)) {
                    ...
                    $model_goods->editGoodsById($goods, $goods_id,$goods['goods_verify']==10?true:false);
                } else {
                    $goods['goods_name']        = $common_array['goods_name'] . ' ' . implode(' ', $value['sp_value']);
                    $goods['goods_price']       = $value['price'];
                    $goods['goods_promotion_price']=$value['price'];
    ...
    public function saveMemberPrice($data){
        ...
        if(!empty($goods_array)){
            foreach ($goods_array as $value){
                $update = array();
                $update['member_price_1'] = $data['memberprice1_'.$value['goods_id']];
                $update['member_price_2'] = $data['memberprice2_'.$value['goods_id']];
                $update['member_price_3'] = $data['memberprice3_'.$value['goods_id']];
                $model_goods->editGoodsById($update, $value['goods_id']);
            }
            return callback(true);
        }
     ...
     * 通过会员等级获取商品会员价格
     */
    public function getGoodsMemberPrice($goods_info, $member_info = array()){
        $goods_info['is_member_price'] = 0;
        if(empty($goods_info['promotion_type']) && $goods_info['is_book'] != '1' && ($goods_info['member_price_1']>0 || $goods_info['member_price_2']>0 || $goods_info['member_price_3']>0)){
            $goods_info['is_member_price'] = 1;
            $level = $member_info['level'];
            if ($level > 0) {
                for($i = $level;$i>0;$i--) {
                    $member_price = $goods_info['member_price_'.$i];
                    if ($member_price > 0 && $member_price < $goods_info['goods_price']) {
                        $goods_info['is_member_price'] = 1;
                        $goods_info['member_price'] = $member_price;
                        $goods_info['promotion_price'] = $member_price;
