...
class hourControl extends BaseCronControl {
        ...
        try {
            ...
            if ($count != 0) {
                for ($i = 0; $i <= $count; $i = $i + $step_num){
                    if (C('dbdriver') == 'mysql') {
                        $goods_list = $model_goods->getGoodsOnlineList(array(), '*', 0, '', "{$i},{$step_num}");
                    } elseif (C('dbdriver') == 'oracle') {
    ...
    public function _xs_update() {
        ...
        try {
            ...
            for ($i = 0; $i <= $count; $i = $i + $step_num){
                if (C('dbdriver') == 'mysql') {
                    $goods_list = $model_goods->getGoodsOnlineList($condition, '*', 0, '', "{$i},{$step_num}");
                } elseif (C('dbdriver') == 'oracle') {
                    //先查出所有goods_id,再使用in查询
                    $condition['goods_state']   = 1;
                    $condition['goods_verify']  = 1;
    ...
    private function _build_goods($goods_list = array()) {
        ...
        foreach ($goods_list as $k => $v) {
            ...
            if ($goods_class[$v['gc_id']]['show_type'] == 1) {
                $index_data['goods_commonid_colorid'] = $v['goods_commonid'].'_'.$v['color_id'];
            }else{
				$index_data['goods_commonid_colorid'] = $v['goods_commonid'];
            }
            ...
            // 判断店铺是否为自营店铺
            $index_data['is_own_shop'] = $v['is_own_shop'];
