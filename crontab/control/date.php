...
class dateControl extends BaseCronControl {
    ...
    public function indexOp() {
        ...
        //门店结算处理
        Model('chain_bill')->createChainBill();

        //关闭退出分销超过N天的用户的分销中心
        $this->_distribute_member_close();
        //清理退出分销超过N天的商品
        $this->_distribute_goods_close();
    }
    ...
    private function _order_book_end_pay_notice() {
        ...
        foreach ($order_book_list as $order_book_info) {
            ...
            if (!empty($order_book_info['book_pay_time'])) {
                ...
                $_goods = $model_order->getOrderGoodsInfo(array('order_id'=> $order_book_info['book_order_id']));
                $_goods['book_amount'] = $_book_info['book_amount'];
                $param['param']['mp_array'] = Logic('wx_api')->getTemplateData($param['code'],$_goods);
    ...
    private function _order_delivery_cancel_del() {
        ...
        for($i = 0; $i < 10; $i++) {
            $condition = array();
            $condition['dlyo_state'] = 10;
            $delivery_list = $model_chain_order->getDeliveryOrderList($condition, 0, 100, 'order_id asc');
            if (!empty($delivery_list)) {
                ...
                if (!empty($order_list)) {
                    ...
                    $del = $model_chain_order->delDeliveryOrder(array('order_id'=>array('in',$order_ids)));
                    if (!$del) {
                        $this->log('删除门店代收订单失败');
                    }
                } else {
    ...
    private function _goods_click_update() {
        ...
        foreach ($data as $key => $val) {
            Model('goods')->editGoodsById(array('goods_click' => $val), $key);
        }
    ...
    private function _arrival_notice() {
        ...
        for ($i = 0; $i <= $times; $i++) {
            ...
            $goods_list = Model('goods')->getGoodsOnlineList(array('goods_id' => array('in', $goodsid_array), 'goods_storage' => array('gt', 0)), 'goods_id,goods_name,goods_price,goods_promotion_price');
            if (empty($goods_list)) continue;
    
            // 需要通知到货的商品
            $goodsid_array = array();
            foreach ($goods_list as $val) {
                if (empty($val['goods_promotion_price'])) $val['goods_promotion_price'] = $val['goods_price'];
                $goodsid_array[$val['goods_id']] = $val;
            }
    
            // 根据商品id重新查询需要通知的列表
            $notice_list = $model_arrtivalnotice->getArrivalNoticeList(array('goods_id' => array('in', array_keys($goodsid_array))), '*');
            if (empty($notice_list)) continue;
            ...
            foreach ($notice_list as $val) {
                ...
                $goods_info = $goodsid_array[$val['goods_id']];
                $goods_info['an_addtime'] = date('Y-m-d H:i:s',$val['an_addtime']);
                $param['param']['mp_array'] = Logic('wx_api')->getTemplateData($param['code'],$goods_info);
                RealTimePush('sendMemberMsg', $param);
            }
    
            // 清楚发送成功的数据
            $model_arrtivalnotice->delArrivalNotice(array('goods_id' => array('in', array_keys($goodsid_array))));
        }
    }
    ...
    private function _calc_real_order_bill($data_bill){
        ...
        $cost_info = $model_store_cost->table('store_cost')->getStoreCostInfo($cost_condition,'sum(cost_price) as cost_amount');
        ...
        //本期应结
        $update['ob_result_totals'] = $update['ob_order_totals'] + $update['ob_rpt_amount'] + $update['ob_order_book_totals'] - $update['ob_offline_totals'] - $update['ob_order_return_totals'] -
     ...
     * 关闭退出分销超过N天的用户的分销中心
     */
    private function _distribute_member_close(){
        $days = MEMBER_DISTRIBUTE_CLOSE * 86400;
     ...
     * 清理退出分销超过N天的商品
     */
    private function _distribute_goods_close(){
        $days = GOODS_DISTRIBUTE_DEL * 86400;
        $model_dis_goods = Model('dis_goods');
        $condition = array();
        $condition['distri_goods_state'] = 2;
        $condition['quite_time'] = array('elt',TIMESTAMP - $days);
        $model_dis_goods->dbDelDistriGoods($condition);
    }
}