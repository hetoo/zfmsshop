...
class minutesControl extends BaseCronControl {
     ...
     * 大转盘活动状态更新
     */
    private  function _update_dial_state(){
        $condition = array();
        $condition['end_time'] = array('elt',TIMESTAMP);
        Model('lottery_dial')->editDialActivity(array('lot_state'=>2),$condition);
        $condition = array();
        $condition['start_time'] = array('egt',TIMESTAMP);
        Model('lottery_dial')->editDialActivity(array('lot_state'=>1),$condition);
    }
    ...
    private function _cron_5($cron = array()) {
        ...
        $groupbuy = Model('groupbuy')->getGroupbuyList($condition, null, 'groupbuy_id asc');
