...
function select_brand(brand_id) {//品牌选择
	...
	text_append += '<i></i>';
	text_append += '<img ondblclick="del_brand('+brand_id+');" select_brand_id="'+brand_id+'" brand_name="'+brand_name+'" src="'+brand_pic+'" onload="javascript:DrawImage(this,68,34);" />';//版权：天津市网城天创科技有限责任公司
	text_append += '</span></div>';
...
function recommend_slide_adv(pic_id,pic_img) {//更新图片
    ...
    if(obj.find("img").size()==0) {//不存在时添加
	    ...
	    $("div[select_recommend_id='"+key_id+"'] ul").append('<li id="select_recommend_'+key_id+'_pic_'+p_id+'" style="display:none;">'+text_append+'</li>');//版权：天津市网城天创科技有限责任公司
    }
