...
$(document).ready(function(){
    ...
    //插入抽奖对话框
    $('#btn_show_special_add_lottery').click(function(){
        html_form('dialog_special_add_lottery', '插入抽奖', $("#_dialog_special_add_lottery").html(), 580);
    });

    $("[nctype='btn_special_lottery']").live('click',function(){
        var goods_list = $(this).parents("div").find("[nctype='_special_lottery_list']");
        var lottery_name = $(this).parents("div").find("[nctype='_input_lottery_name']").val();
        var lottery_type = $(this).parents("div").find("[nctype='_input_lottery_type']").val();
        var start_time = $(this).parents("div").find("[nctype='start_time']").val();
        var end_time = $(this).parents("div").find("[nctype='end_time']").val();

        if(lottery_name != '' || lottery_type != '' || start_time != '' || end_time != '') {
            goods_list.load('index.php?act=cms_special&op=get_lottery_list&'+$.param({lottery_name:lottery_name,lottery_type:lottery_type,start_time:start_time,end_time:end_time}));
        }
    });
