...
$(function(){
    ...
    /* 商品视频ajax上传 */
    $('#goods_video').fileupload({
        dataType: 'json',
        url: ADMIN_SITE_URL + '/index.php?act=lib_goods&op=video_upload&upload_type=uploadedfile',
        formData: {name:'goods_video'},
        add: function (e,data) {
            $('video[nctype="goods_video"]').attr('poster', ADMIN_TEMPLATES_URL + '/images/loading.gif');
            $('video[nctype="goods_video"]').attr('src', "");
            data.submit();
        },
        pasteZone: null,
            ...
            if (typeof(param.error) != 'undefined') {
                alert(param.error);
                $('video[nctype="goods_video"]').attr('poster',DEFAULT_GOODS_VIDEO);
                $('video[nctype="goods_video"]').attr('src', "");
            } else {
                $('input[nctype="goods_video"]').val(param.name);
                $('video[nctype="goods_video"]').attr('poster', "");
                $('video[nctype="goods_video"]').attr('src',param.goods_video);
            }
        }
    });
    /* 商品图片ajax上传 */
    $('#goods_image').fileupload({
        dataType: 'json',
        url: ADMIN_SITE_URL+'/index.php?act=lib_goods&op=image_upload&upload_type=uploadedfile',
    ...
    // 商品视频使用
    $('a[nctype="show_video"]').unbind().ajaxContent({
        event:'click', //mouseover
        loaderType:"img",
        loadingMsg:ADMIN_TEMPLATES_URL+"/images/loading.gif",
        target:'#video_demo'
        ...
        $(this).hide();
        $('a[nctype="del_goods_video_demo"]').show();
    });
    $('a[nctype="del_goods_video_demo"]').unbind().click(function(){
        $('#video_demo').html('');
        $(this).hide();
        $('a[nctype="show_video"]').show();
    });
    // 商品描述使用
    ...
    $('#add_album').fileupload({
        dataType: 'json',
        url: ADMIN_SITE_URL+'/index.php?act=lib_goods&op=image_upload',
        formData: {name:'add_album'},
        add: function (e,data) {
            $('i[nctype="add_album_i"]').removeClass('icon-upload-alt').addClass('icon-spinner icon-spin icon-large').attr('data_type', parseInt($('i[nctype="add_album_i"]').attr('data_type'))+1);//版权：天津市网城天创科技有限责任公司
            data.submit();
        },
        done: function (e,data) {
    ...
    $('a[nctype="mb_add_img"]').click(function(){
        $('a[nctype="meat_cancel"]').click();
        $('div[nctype="mea_img"]').show().load(ADMIN_SITE_URL+'/index.php?act=lib_goods&op=pic_list&item=mobile');
    });
    // 关闭 图片选择
    ...
    $('div[nctype="mobile_pannel"]').on('click', 'a[nctype="mp_rpl"]', function(){
        $('a[nctype="meat_cancel"]').click();
        $('div[nctype="mea_img"]').show().load(ADMIN_SITE_URL+'/index.php?act=lib_goods&op=pic_list&item=mobile&type=replace');
    });
    // 插图图片
...
function insert_video(name, src) {
    $('input[nctype="goods_video"]').val(name);
    $('video[nctype="goods_video"]').attr('poster', "");
    $('video[nctype="goods_video"]').attr('src',src);
}
...
function insertBrand(param, search) {
    ...
    $.each(param, function(i, n){
        $('<li data-id="' + n.brand_id + '" data-name="' + n.brand_name + '"><em>' + n.brand_initial + '</em>' + n.brand_name + '</li>').appendTo(_ul);//版权：天津市网城天创科技有限责任公司
    });
