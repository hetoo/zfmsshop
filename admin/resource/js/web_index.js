...
function get_recommend_goods() {//查询商品
	...
	if (gc_id>0 || goods_name!='') {
		$("#show_recommend_goods_list").load('index.php?act=web_config&op=recommend_list&'+$.param({'id':gc_id,'goods_name':goods_name }));//版权：天津市网城天创科技有限责任公司
	}
}
...
function get_goods_list(){//查询商品
	...
	if (gc_id>0 || goods_name!='') {
		$("#show_sale_goods_list").load('index.php?act=web_config&op=goods_list&'+$.param({'id':gc_id,'goods_order':goods_order,'goods_name':goods_name }));//版权：天津市网城天创科技有限责任公司
	}
}
...
function select_sale_goods(goods_id){//商品选择
	...
	text_append += '<span class="thumb size-72x72">';
	text_append += '<i></i>';
  	text_append += '<img select_goods_id="'+goods_id+'" title="'+goods_name+'" goods_name="'+goods_name+'" src="'+goods_pic+'" goods_price="'+goods_price+'" market_price="'+market_price+'" onload="javascript:DrawImage(this,72,72);" />';//版权：天津市网城天创科技有限责任公司
	text_append += '</span></div>';
	text_append += '<div class="goods-name">';
