...
class cms_specialControl extends SystemControl{
     ...
     * 获取抽奖列表
     */
    public function get_lottery_listOp(){
        $condition = array();
        $condition['lot_state'] = array('in',array(0,1));
        if(trim($_GET['lottery_name']) != ''){
            $condition['lot_name'] = trim($_GET['lottery_name']);
        }
        if(in_array(intval($_GET['lottery_type']),array(0,1))){
            $condition['lot_type'] = intval($_GET['lottery_type']);
        }
        $pdates = array();
        if (strlen($q = trim((string) $_REQUEST['add_time'])) && ($q = strtotime($q . ' 00:00:00'))) {
            $pdates[] = "add_time >= {$q}";
        }
        ...
        $model_lottery = Model('lottery_dial');
        $dial_list = $model_lottery->getListDialActivity($condition, 6);
        Tpl::output('dial_list',$dial_list);
        Tpl::output('show_page',$model_lottery->showpage());
        Tpl::showpage('cms_special.dial','null_layout');
    }
