...
$(function(){
    $("#flexigrid").flexigrid({
        ...
        colModel : [
            ...
            {display: '商品视频', name : 'goods_video', width : 60, sortable : true, align: 'center'},
...
function fg_goods_add(id) {
        $.getJSON('index.php?act=goods&op=goods_add', {id:id}, function(data){
            if (data.state) {
                showSucc(data.msg)
            } else {
                showError(data.msg)
...
function fg_show_video(ids) {
      _uri = "index.php?act=goods&op=goods_show_video&id=" + ids;
