        ...
        if (chksubmit()) {
            ...
            if ($detail_array['pay_time'] > 0) {
                ...
                if ($_POST['admin_state'] == '3') {
                    ...
                    $refund_array['seller_message'] = $return['seller_message'].'(商家同意,但管理员不同意退款)';
                    ...
                    if ($return['order_lock'] == '2') $model_refund->editOrderUnlock($order_id);//订单解锁
                    ...
                    $param['param'] = array(
                        'refund_url' => urlShop('member_refund', 'view', array('refund_id' => $return['refund_id'])),
                        'refund_sn' => $return['refund_sn']
                    );
                    $return['msg'] = '管理员不同意退款';//状态描述
                    $param['param']['mp_array'] = Logic('wx_api')->getTemplateData($param['code'],$return);
                    RealTimePush('sendMemberMsg', $param);
    
                    $this->log('退货处理，退货编号'.$refund['refund_sn']);
