...
class SystemControl{
     ...
     *
     */
    protected final function setFlexigridInfo() {
        $act = $_GET['act'] ? $_GET['act'] : 'index';
        $shopnc_type = $_POST['shopnc_type'];
        if ($shopnc_type == 'shopnc_js') {
            $shopnc_op = $_POST['shopnc_op'];
            $key = $shopnc_op;
            $page = intval($_POST['rp']);//每页数量
            $curpage = intval($_POST['curpage']);//当前页码
            $_SESSION['shopnc_admin'][$key]['page'] = $page;
            $_SESSION['shopnc_admin'][$key]['curpage'] = $curpage;
            if ($key == '_'.$act.'index') {
                $key = '_'.$act.$act;
                $_SESSION['shopnc_admin'][$key]['page'] = $page;
                $_SESSION['shopnc_admin'][$key]['curpage'] = $curpage;
            ...
            $key = '_'.$act.$op;
            $page = intval($_SESSION['shopnc_admin'][$key]['page']);
            $curpage = 1;
            if ($act == $_SESSION['shopnc_last']) $curpage = intval($_SESSION['shopnc_admin'][$key]['curpage']);
            Tpl::output('shopnc_page',$page);
            Tpl::output('shopnc_curpage',$curpage);
            Tpl::output('shopnc_op',$key);
        }
        if ($act != 'index') $_SESSION['shopnc_last'] = $act;
    }
    ...
    protected final function sublink($links = array(), $actived = '', $file='index.php') {
        ...
        foreach ($links as $k=>$v) {
            ...
            $key = '_'.$array['act'].$array['op'];
            if ($array['op'] != $actived) $_SESSION['shopnc_admin'][$key]['curpage'] = 1;
            if ($actived != 'index') $_SESSION['shopnc_admin']['_'.$array['act'].'index']['curpage'] = 1;
        }
