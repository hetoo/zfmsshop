...
class Model{
    ...
    public function __call($method,$args) {
        if(in_array(strtolower($method),array('table','order','where','on','limit','having','lock','group','master','distinct','pk','key'),true)) {
            ...
            return $this;
        }elseif(in_array(strtolower($method),array('union'),true)){
            if(in_array(strtolower($args[0]),array('index','array'),true)){
                if($args[0] == 'index'){
                ...
                $error = 'Model Error:  Function '.$method.' parameter error!';
                throw_exception($error);
                return;
            }
            $this->options[strtolower($method)] =   $args;
            return $this;
