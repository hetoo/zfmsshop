...
class Db{
    ...
    public function select($options = array()) {
        ...
        if ($this->subquery) {
            $sql = 'SELECT '.$this->subquery.' FROM ('.$sql.') a';
            $this->subquery = '';
        }
    ...
    protected function parseSql($sql, $options = array()){
        ...
        $sql   = str_replace(
            ...
            array(
                isset($options['union'])?$this->parseUnion($options, $where):$this->parseTable($options,'select'),
                $this->parseDistinct(isset($options['distinct'])?$options['distinct']:false),
                $this->parseField(isset($options['field'])?$options['field']:'*'),
                isset($options['union'])?'':$this->parseJoin(isset($options['on'])?$options:array()),
                isset($options['union'])?'':$where,
    ...
    protected function parseUnion($options, $where){
        $return_union = '';
        $table_arr = explode(',', $options['table']);
        if(count($table_arr) == 1){
            $sql_tpm = 'SELECT * FROM %TABLE%%WHERE%%GROUP%';
            $union = $options['union'];
            $union_table_type = $union[0];
            $union_sqls = array();
            $_table = trim(str_replace(DBPRE,'',$options['table']),'`');
            if(is_string($union_table_type) && strtolower($union_table_type) == 'index'){
                if(is_numeric($union[1]) && $union[1] > 0){
                    $last_index = $union[1];
                    for($i = 1; $i <= $last_index; $i++){
                        $table_name = $_table.'_'.$i;
                        $union_sqls[] = str_replace (
                            array('%TABLE%','%WHERE%','%GROUP%'),
                            array(
                                $this->_parseUnionTable($table_name),
                                $where,
                                $this->parseGroup(isset($options['group'])?$options['group']:''),
                            ),$sql_tpm);
                    }
                }
            }elseif(is_string($union_table_type) && strtolower($union_table_type) == 'array'){
                if(!empty($union[1])){
                    $union_tables = $union[1];
                    foreach($union_tables as $val){
                        $union_sqls[] = str_replace (
                            array('%TABLE%','%WHERE%','%GROUP%'),
                            array(
                                $this->_parseUnionTable($val),
                                $where,
                                $this->parseGroup(isset($options['group'])?$options['group']:''),
                            ),$sql_tpm);
                    }
            ...
            if(!empty($union_sqls)){
                $_sql = str_replace (
                    array('%TABLE%','%WHERE%','%GROUP%'),
                    array(
                        $this->_parseUnionTable($_table),
                        $where,
                        $this->parseGroup(isset($options['group'])?$options['group']:''),
                    ),$sql_tpm);
                $return_union = '('.$_sql.$this->_parseUnionAll($union[2]).implode($this->_parseUnionAll($union[2]),$union_sqls).') '.'union_'.$_table;
            }
        }
        return $return_union;
    }

    protected function _parseUnionTable($table){
        return '`'.DBPRE.$table.'` '.$table;
    }

    protected function _parseUnionAll($is_all = true){
        return $is_all?' UNION ALL ':' UNION ';
    }
